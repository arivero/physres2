...
The six-texture-zero Fritzsch ansatz gives the analogous relation
$|V_{cb}| = |\sqrt{m_s/m_b} - e^{i\psi}\sqrt{m_c/m_t}|$; numerically
$\sqrt{m_s/m_b} = 0.149$ and $\sqrt{m_c/m_t} = 0.088$,
so $|V_{cb}| \geq 0.061$ for any phase~$\psi$, overshooting
the PDG value $0.0422$ by $40\%$.  This structural failure of
the original Fritzsch texture (known since the 1990s) signals
that the 2--3 sector requires a modified texture or
running effects beyond tree level.
