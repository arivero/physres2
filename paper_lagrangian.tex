...
the magnetic Koide predicts $m_d$ and hence $m_u/m_d = 0.453$.
...
and $m_b$; the magnetic Koide determines $m_d$ (hence $m_u$) from
...
&\xrightarrow{\;Q(0,\sqrt{m_u{+}m_d},\sqrt{m_s})\approx 2/3\;}
...
&\xrightarrow{\;Q(1/\sqrt{m_d},1/\sqrt{m_s},1/\sqrt{m_b})\approx 2/3\;}
...
$\langle z_k^2\rangle = z_0^2$; plus the magnetic Koide from
...
The isospin and magnetic Koide are approximate ($Q \approx 2/3$,
bloomed from the exact seed); the remaining three are exact.
...
\midrule
Isospin seed: $m_s \approx (2{+}\sqrt{3})^2(m_u{+}m_d)$ & $m_s$ from $m_u{+}m_d$ & $Q(0,\sqrt{m_u{+}m_d},\sqrt{m_s}) \approx 2/3$; bloomed \\
...
Magnetic: $Q(1/m_d, 1/m_s, 1/m_b) = 2/3$ & $m_d$ from $m_s, m_b$ & Seiberg seesaw \\
...
The quark masses appearing in the Koide triples are
...
the Koide ratios sharply: $Q(-s,c,b)$ worsens from $1.2\%$ to $3.2\%$,
and $Q(c,b,t)$ from $0.4\%$ to $6.2\%$.
The isospin seed $Q(0,\sqrt{m_u{+}m_d},\sqrt{m_s})$ is immune to
running because all entries share the same anomalous dimension and
...
None of the quark triples sits exactly at $Q = 2/3$:
...
the bion gives $m_b$, and the magnetic Koide gives $m_d$ (hence $m_u$).
...
to dual charges $\xi_k = 1/\sqrt{m_k}$, with
$Q(\xi_1,\xi_2,\xi_3) = 1 - 2e_1 e_3/e_2^2$
where $e_i$ are the elementary symmetric polynomials of the $\zch_k$.
In the angular parametrisation, the condition
$Q(\zch) = Q(\xi) = 2/3$ simultaneously reduces to
\begin{equation}
  \cos 3\delta = \frac{5\sqrt{2}}{8}\,,
\end{equation}
an isolated angle distinct from the seed ($\delta = 3\pi/4$).
The seed self-duality $Q(0,a,b) = Q(0,1/a,1/b)$ is a separate
algebraic identity.
For the physical down-type quarks $(d,s,b)$,
$Q(\xi) = 0.665$ ($0.22\%$ from $2/3$)
while $Q(\zch) = 0.732$---far from $2/3$.
The seesaw thus creates an independent near-Koide condition
on the magnetic charges, constraining $m_d$ from $(m_s, m_b)$.
...
The chiral spectrum from the D9-brane sector is
$3\times[(\mathbf{10},\mathbf{1}) + (\bar{\mathbf{5}},\mathbf{3})
  + (\mathbf{5},\bar{\mathbf{3}})]$,
where the $\mathbf{10}$ is the antisymmetric two-index
representation of $SU(5)_{\mathrm{f}}$, arising from the
SO-type orientifold projection ($\varepsilon = +1$) forced by
the odd dimension of the charge-0 blocks.
Whether the symmetric~$\mathbf{15}$ can be obtained---e.g.\
via an O$^+$-plane (Sp-type projection) with even-dimensional
spectator blocks, or from unoriented string sectors---remains
an open question.
...
constraint $\langle z_k^2\rangle = z_0^2$, and the magnetic Koide
...
$m_c, m_b$), the magnetic Koide ($m_d$ from $m_s, m_b$),
