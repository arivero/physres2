...
mass charges $m_k = (z_0 + z_k)^2$. From the single input $m_u + m_d$
(equivalently $m_\pi$ via GMOR), the isospin seed predicts $m_s$,
the O'Raifeartaigh chain predicts $m_c = 1301\MeV$ (+2.0\%),
$m_b = 4233\MeV$ (+1.3\%), and $m_t = 171\,250\MeV$ ($-$0.9\%);
the dual mass-charge identity predicts $m_d$ and hence $m_u/m_d = 0.453$.
The lepton sector predicts
$m_\tau = 1776.97$ MeV (+0.006\%), and the GST relation predicts
the Cabibbo angle.
In total seven of the Standard Model's
thirteen charged-fermion-sector parameters are determined, reducing the
free parameter count from thirteen to six.
\end{abstract}
...
deviation). With the isospin seed giving $m_s$ from $m_u + m_d$,
the entire heavy quark spectrum descends from a single input.

The lepton sector is governed by the same algebraic structure.
In the sBootstrap, charged leptons are the fermionic partners of
mesons; their mass matrix therefore inherits the O'Raifeartaigh
form, stabilized at the same $gv/m = \sqrt{3}$.  Because the
lepton sector has no magnetic bion mechanism, the physical spectrum
stays close to the O'Raifeartaigh seed. The mass-charge identity
...
Two further predictions follow from the Lagrangian. An NMSSM singlet $S$
(separate from the Seiberg Lagrange multiplier $X$)
generates a Higgs quartic through $\lambda_S S H_u \cdot H_d$.
At $\tan\beta = 1$ the tree-level Higgs mass is
$m_h = \lambda_S v/\sqrt{2}$, and
$m_h = 125.35\GeV$ fixes $\lambda_S = 0.72$. The Cabibbo angle
emerges from the seesaw-inverted mass hierarchy: the Fritzsch texture
of the mass matrix $M \propto \hat{m}^{-T}$ gives the
Gatto--Sartori--Tonin relation
$\sin\theta_C = \sqrt{m_d/m_s} = 0.224$ (PDG: 0.224).
...
CKM angles. In this theory, the isospin seed determines
$m_s$ from $m_u + m_d$; the O'Raifeartaigh mass ratio determines
...
and $m_b$; the dual mass-charge identity determines $m_d$ (hence $m_u$) from
$m_s$ and $m_b$; the mass-charge identity determines $m_\tau$ from $m_e$
and $m_\mu$; and the Gatto--Sartori--Tonin relation determines
$\theta_{12}$ from $m_d$ and $m_s$. Seven parameters are eliminated. The
remaining six free parameters are $m_u$, $m_e$, $m_\mu$,
...
lepton sector, whose O'Raifeartaigh structure mirrors the quark sector
through the Miyazawa superpartner identification. Section~\ref{sec:electroweak} discusses the NMSSM Higgs
...
parameter budget.
Section~\ref{sec:strings} offers a speculative UV interpretation
as a low-scale Type~I superstring on $T^6/\mathbb{Z}_3$; this section
is not required for the mass predictions and may be skipped by readers
uninterested in string constructions.
Section~\ref{sec:open} discusses the remaining open problems ---
the 15 versus $\overline{10}$ Pauli conflict, the
nonperturbative bloom mechanism, and the mesino mass hierarchy ---
...
\paragraph{Cartan-plane geometry.}
The mass triple defines a vector in the $(\lambda_3,\lambda_8)$ Cartan
plane of $SU(3)$:
$\vec{r} = \sum_{k} \zch_k\,\vec{w}_k$,
where $\vec{w}_k$ are the fundamental weights.
Since $\sum \vec{w}_k = 0$ the constant part drops out, and a
standard trigonometric reduction gives
\begin{equation}
\vec{r} = \frac{z_0\sqrt{6}}{2}
\bigl(\sin(\delta+\tfrac{\pi}{3}),\;
      \cos(\delta+\tfrac{\pi}{3})\bigr)\,,
\qquad \theta = \frac{\pi}{6} - \delta\,,
\end{equation}
where $\theta$ is the polar angle of~$\vec{r}$.
This is an exact identity for \emph{all}~$\delta$ and~$z_0$:
the bloom is a rigid rotation in the Cartan plane at fixed
radius $R = z_0\sqrt{6}/2$, with no radial component.
...
(v)~a $\delta$-dependent potential from the three-instanton operator
$(\det M^{(3)})^3/\Lambda_{\text{eff}}^{18}$, which selects
discrete bloom angles (Section~\ref{sec:bloom}).

\subsection{The flavour-universal Yukawa identity}

The Seiberg seesaw and the standard Yukawa coupling combine to
produce a flavour-independent product. Each confined quark~$j$
has a Yukawa coupling $y_j = \sqrt{2}\,m_j/v_{\text{EW}}$
(at $\tan\beta = 1$), while the seesaw fixes the meson VEV
$\langle M_j\rangle = C/m_j$ with
$C = \Lambda^2\bigl(\prod_k m_k\bigr)^{1/N_f}$ flavour-independent.
Their product is
\begin{equation}
  y_j\,\langle M_j\rangle
  = \frac{\sqrt{2}\,m_j}{v_{\text{EW}}}\;\frac{C}{m_j}
  = \frac{\sqrt{2}\,C}{v_{\text{EW}}}\,,
\end{equation}
which is the same for every confined flavour: the quark-mass
dependence cancels exactly. Evaluating for the light block
$(u,d,s)$ with $C = 8.8 \times 10^5\MeV^2$:
\begin{equation}
  y_j\,\langle M_j\rangle
  = \frac{\sqrt{2} \times 8.8 \times 10^5}{2.46 \times 10^5}
  = 5.07\MeV
  \qquad\text{(all confined flavours)}\,.
\end{equation}

This identity has two consequences. First, it shows that the Yukawa
couplings are \emph{not free parameters}: they are determined by
$C$ and $v_{\text{EW}}$, both of which are fixed by the Lagrangian.
Second, because the Higgs--meson vertex $y_j\langle M_j\rangle$
is flavour-universal, the Glashow--Weinberg
condition~\cite{UTfit} for the absence of tree-level
Higgs-mediated FCNCs is satisfied automatically.
This is not a tuning; it is a structural consequence of the seesaw.
...
In the colour-flavour locked phase with $N_f = N_c$, each monopole
couples to exactly one flavour, so the contributions enter
\emph{additively} in the superpotential:
$W_{\mathrm{mon}} = \zeta\,e^{-S_0/N_c}\sum_a \sqrt{m_a}$.
(For $N_f > N_c$ the mass dependence would be multiplicative,
$\prod_k \sqrt{m_k}$, and the sum-of-square-roots structure
would not arise.)
The leading bion correction to the K\"ahler potential has the form
\begin{equation}
K_{\mathrm{bion}}
...
Setting $S_{\mathrm{bloom}} = 2\,S_{\mathrm{seed}}$ gives
$-\sqrt{m_s} + \sqrt{m_c} + \sqrt{m_b}
  = 2(\sqrt{m_s} + \sqrt{m_c})$,
hence the $v_0$-doubling condition:
\begin{equation}
\boxed{\sqrt{m_b}
= 3\sqrt{m_s} + \sqrt{m_c}\,.}
\end{equation}
The integer coefficient $3 = 2 + 1$: the factor~$2$ comes from
$z_0$-doubling; the extra~$1$ from the sign flip ($\sqrt{m_s}$
enters the seed with $+1$ but the bloom with $-1$, adding
$2\sqrt{m_s}$ to the right-hand side).
...
$172.76 \pm 0.30\GeV$ is the pole mass. The two differ by the
...
\subsection{The dual mass-charge identity from the Seiberg seesaw}

The Seiberg seesaw (Section~\ref{sec:lagrangian}) maps quark masses
$m_j$ to meson VEVs $\langle M_j\rangle = C/m_j$, inverting the mass
hierarchy. This inversion acts on the mass charges: if the direct
charges are $\zch_k = \sqrt{m_k}$, the \emph{dual charges} are
$\xi_k \propto 1/\sqrt{m_k}$. Defining
$\xi_0 = \tfrac{1}{3}\sum_k\xi_k$, the dual mass-charge identity is
\begin{equation}
  \langle\xi_k^2\rangle = \xi_0^2\,,
  \qquad\text{equivalently}\quad
  \frac{\sum_k 1/m_k}{\bigl(\sum_k 1/\sqrt{m_k}\bigr)^2}
  = \frac{2}{3}\,.
\end{equation}

At the O'Raifeartaigh seed, one mass vanishes:
$(m_1,m_2,m_3) = (0,m_-,m_+)$. In this limit the direct and dual
identities coincide: with $\zch_1 = 0$,
\begin{equation}
  \frac{m_- + m_+}{(\sqrt{m_-}+\sqrt{m_+})^2}
  = \frac{1/m_- + 1/m_+}{(1/\sqrt{m_-}+1/\sqrt{m_+})^2}\,,
\end{equation}
since both simplify to $(m_- + m_+)/(m_- + m_+ + 2\sqrt{m_- m_+})$
when the zero-mass entry is absent from all sums.
At $gv/m = \sqrt{3}$ both equal $2/3$ exactly: the seed is
\emph{self-dual}.

After the bloom ($m_1 > 0$), the direct and dual ratios diverge.
For the down-type quarks $(d,s,b)$, the direct ratio
$\langle\zch_k^2\rangle/\zch_0^2 = 0.732$ is far from unity,
while the dual ratio
$\langle\xi_k^2\rangle/\xi_0^2 = 0.665/0.667 = 0.997$
deviates by only $0.22\%$.
The seesaw preserves the mass-charge identity much better than
the direct masses do, because it maps the bloomed spectrum back
toward the seed hierarchy $m_{\rm light} \ll m_{\rm heavy}$.

Setting $\langle\xi_k^2\rangle = \xi_0^2$ exactly on $(d,s,b)$,
and taking $m_s$ and $m_b$ as inputs, determines $m_d$:
\begin{equation}
  m_d^{\rm pred} = 4.605\MeV
  \qquad(\text{PDG:}\; 4.67 \pm 0.48\MeV,\;\; -0.13\sigma)\,.
\end{equation}

This prediction is a direct consequence of the Lagrangian:
the seesaw $\langle M_j\rangle = C/m_j$ follows from the
$F$-term equations $\partial W/\partial M_j = 0$
of the superpotential
$W = \mathrm{Tr}(\hat m\,M) + X(\det M - \Lambda^6)$.
The dual mass-charge identity is therefore not an independent
assumption but an algebraic property of the seesaw vacuum.

\subsection{The complete mass chain}

Five quark masses from one input:
\begin{equation}
\boxed{
\begin{aligned}
m_u + m_d &= 6.83\;\mathrm{MeV} \quad\text{(input)}\\[4pt]
&\xrightarrow{\;\langle z_k^2\rangle \approx z_0^2\text{ on }(0,m_u{+}m_d,m_s)\;}
m_s = 95.1\;\mathrm{MeV} \quad[+1.8\%]\\[4pt]
...
m_t = 171{,}250\;\mathrm{MeV} \quad[-0.9\%]\\[4pt]
&\xrightarrow{\;\langle\xi_k^2\rangle \approx \xi_0^2\text{ on }(d,s,b)\;}
m_d = 4.6\;\mathrm{MeV} \quad[-1.5\%]
\end{aligned}
}
\end{equation}

The chain spans a factor of $2.5 \times 10^4$ in mass
(from $m_u + m_d$ to $m_t$),
controlled by four structural constants of the Lagrangian: the
isospin seed ratio, the
O'Raifeartaigh mass ratio $R_{\mathrm{OR}}$, the bion doubling
(with coefficient $N_c = 3$), and the eigenvalue sum rule
$\langle z_k^2\rangle = z_0^2$; plus the dual mass-charge identity from
the Seiberg seesaw. None of these is fitted; each is an algebraic
consequence of the superpotential at its stabilised vacuum.
The isospin and dual mass-charge identity are approximate
($\langle z_k^2\rangle/z_0^2 \approx 1$, bloomed from the exact
seed); the remaining three are exact.
...
\subsection{The O'Raifeartaigh structure for leptons}

In the sBootstrap, the charged leptons are the fermionic partners
of the mesons: they sit in the same supermultiplets split by
SUSY breaking at scale~$\fpi$.  The same O'Raifeartaigh
algebraic structure that generates the quark mass hierarchy
should therefore apply to the lepton sector, with three chiral
superfields $L_1, L_2, L_3$ playing the role of the lepton
generation eigenstates.  The identification of these fields
with specific meson superfields (i.e.\ the supermultiplet
restoration problem) remains open
(Section~\ref{sec:open}).

\subsection{O'Raifeartaigh deformation}

The O'Raifeartaigh superpotential for the lepton sector has
the same algebraic form as the quark sector
...
The K\"ahler potential has the same pole structure:
\begin{equation}
K_{\text{lepton}} = |X_L|^2\!\left(1 - \frac{|X_L|^2}{3\mu_K^2}\right)
  + |L_1|^2 + |L_2|^2 + |L_3|^2\,,
\end{equation}
with $\mu_K = \sqrt{2}\,m/g$.
The spectrum is precisely the SU(3) O'Raifeartaigh spectrum of
Section~\ref{sec:lagrangian}, plus one extra massless
...
This algebraic identity between the quark and lepton sectors
is the \emph{consequence} of the Miyazawa superpartner
identification: the leptons and mesons sit in the same
supermultiplets, so their mass matrices share the same
O'Raifeartaigh structure. The different mass scale
($m_{\text{OR}}^{\text{leptons}} = 471\MeV$ versus
$m_{\text{OR}}^{\text{quarks}} = 93\MeV$) reflects the
Seiberg seesaw inversion $\langle M_j\rangle \propto 1/m_j$,
which maps the quark mass hierarchy to the inverse meson
mass hierarchy.
...
Because the lepton sector has no magnetic bion mechanism (the
bion-driven $v_0$-doubling requires $N_c = 3$ color, absent for
leptons), the physical spectrum stays close to the
...
\paragraph{Neutrino mass (schematic).}
The pseudo-modulus $L_2$ is massless at tree level. If a higher-order
coupling $L_1 L_2 L_3/\Lambda_L^3$ is present (as would arise from
a confining UV completion), the Coleman--Weinberg mass is
suppressed by $(m/\Lambda_L)^6$:
$$
  m_{L_2} \sim \frac{g^2 m}{4\pi}
    \left(\frac{m}{\Lambda_L}\right)^{\!6}\,.
$$
With $m \sim z_0^2 \approx 314\MeV$ and
$g \sim O(1)$, a scale $\Lambda_L \sim 9$--$12\GeV$
places $m_{L_2}$ in the atmospheric neutrino window $9$--$50$\,meV.
The suppression is a power law, not an exponential.
Whether this pseudo-modulus can be identified with a
physical neutrino depends on its quantum numbers under the
electroweak group.

\paragraph{Quark--lepton mixing contrast.}
In the quark sector, the Seiberg
seesaw $\langle M_j\rangle = C/m_j$ creates a hierarchical
texture (the Fritzsch matrix) that produces small CKM angles
via $\sin\theta_C = \sqrt{m_d/m_s}$.  In the lepton sector,
if the cubic coupling $L_1 L_2 L_3$ is present, it treats
all three fields democratically (invariant under~$S_3$),
providing a natural source of large PMNS mixing angles:
\begin{center}
\begin{tabular}{lll}
\toprule
Sector & Texture & Mixing \\
\midrule
Quarks & Seiberg seesaw (hierarchical) & small CKM \\
Leptons & $L_1 L_2 L_3$ coupling (democratic) & large PMNS \\
\bottomrule
\end{tabular}
\end{center}
...
The empirical relation $\sin\theta_C \approx \sqrt{m_d/m_s}$, noticed
by Gatto, Sartori and Tonin~\cite{GST1968} shortly after the
Cabibbo hypothesis, was one of the original motivations for quark
compositeness --- the idea that if the Cabibbo angle is a ratio of
constituent masses, the quarks themselves may have substructure.
In the sBootstrap the substructure is not preons but the SQCD
seesaw: the Fritzsch texture emerges from the inverse mass hierarchy
$\langle M_j\rangle \propto 1/m_j$, and the Cabibbo angle becomes
a derived quantity of the Lagrangian.
...
The ISS model with $N_f = 4$ confines four flavors $(u,d,s,c)$.
The CKM matrix is determined by off-diagonal entries in the UV mass
matrix $\hat\mu$ \emph{within} each electroweak sector: cross-block
entries connecting up-type to down-type give zero CKM mixing
(the Schur complement of the seesaw-inverted matrix is diagonal).
With only four flavors, the maximum CKM structure is $2\times 2$
(the Cabibbo angle). The 2--3 mixing $V_{cb}$ requires the
top and bottom quarks, but $N_f = 5$ lies outside the
ISS conformal window $N_c < N_f < 3N_c/2 = 4.5$.
The six-texture-zero Fritzsch ansatz, applied formally to
the $3\times 3$ case, gives
$|V_{cb}| = |\sqrt{m_s/m_b} - e^{i\psi}\sqrt{m_c/m_t}| \geq 0.064$,
overshooting the PDG value $0.0422$ by more than $50\%$.
This structural failure (known since the 1990s)
signals that the 2--3 sector requires either a modified texture,
RG running, or a separate mechanism for the top--bottom coupling.
The $\mathbb{Z}_3$ orbifold of Section~\ref{sec:strings} provides
a candidate: Yukawa couplings between different twisted sectors are
suppressed by $\epsilon_1 = e^{-2\pi\sqrt{3}/12} \approx 0.40$
per $T^2$ factor.  The product over the three tori in $T^6$
gives $\epsilon_1^3 = e^{-\pi\sqrt{3}/2} \approx 0.066$, which is
in the right range for~$|V_{cb}|$ but overshoots by~$56\%$.
The precise value depends on the worldsheet instanton
calculation and the fixed-point assignment of quarks --- details
that await a concrete Type~I model.
...
\subsection{Spectrum flow}

The spectrum evolves through six stages as successive Lagrangian
terms are switched on. Table~\ref{tab:flow} tracks the scalar
and fermion content at each stage, all evaluated at $t = \sqrt{3}$.
Mass-squareds are in units of~$m^2$ (the chiral mass parameter).

\begin{table}[ht]
\centering
\small
\begin{tabular}{p{3.0cm}p{3.8cm}p{4.8cm}l}
\toprule
Stage & Fermions & Scalars & $\mathrm{STr}$ \\
\midrule
\textbf{0.}~Full SUSY ($y=0$)
  & $(0,\, m_-,\, m_+)$
  & $\sigma = \pi = (0,\, m_-^2,\, m_+^2)$; exact pairs
  & $0$ \\[4pt]
\textbf{1.}~$F$-breaking ($f\neq 0$)
  & unchanged
  & $B$-term splits: $\pi_k^2 < m_k^2 < \sigma_k^2$; splittings $\propto y$
  & $0$ \\[4pt]
\textbf{2.}~CW stabilization
  & unchanged
  & $\sigma_0 \to m_{\text{CW}} \approx 13\MeV$; $\pi_0$ remains light
  & $0$ \\[4pt]
\textbf{3.}~Bion $K$
  & $v_0$-doubling: $(m_s,m_c) \to (m_s,m_c,m_b)$
  & scalars track fermion masses
  & $0$ \\[4pt]
\textbf{4.}~3-instanton
  & $\delta$ selected; bloom angle fixed
  & angular potential adds $\delta$-dependent shift
  & $0$ \\[4pt]
\textbf{5.}~Soft ($\tilde m^2 = \fpi^2$)
  & unchanged
  & all scalars $\to m^2 + \fpi^2$; mesons at $\sqrt{2}\,\fpi$
  & $18\,\fpi^2$ \\
\bottomrule
\end{tabular}
\caption{Spectrum flow from SUSY to the physical vacuum.
$m_\pm = (2 \pm \sqrt{3})\,m$, so
$m_-^2 = (7{-}4\sqrt{3})\,m^2 \approx 0.072\,m^2$,
$m_+^2 = (7{+}4\sqrt{3})\,m^2 \approx 13.93\,m^2$.
The supertrace vanishes at every stage except the last, where
only the nine soft meson masses contribute.}
\label{tab:flow}
\end{table}

At Stage~0, the three chiral multiplets form exact
$\mathcal{N}=1$ supermultiplets: each Weyl fermion of mass~$m_k$
pairs with a complex scalar (one $\sigma$ and one $\pi$ component)
of mass-squared~$m_k^2$. At Stage~1, the $B$-term
$B_{11} = 2gf$ splits each pair into a heavier $\sigma$ and a lighter
$\pi$ partner while preserving the supertrace. The ordering
$\pi_k^2 < m_k^2 < \sigma_k^2$ holds at all~$y < 1/2$
and $t = \sqrt{3}$, with splittings proportional to~$y$.
The CW potential (Stage~2) lifts only the goldstino multiplet's
flat directions; Stages~3--4 constrain the mass ratios
(bion: radial; instanton: angular) without altering the
multiplet structure. Only at Stage~5 does the supertrace become
nonzero: $\mathrm{STr}[M^2] = 18\,\fpi^2$, because the soft
scalar masses $\tilde m^2 = \fpi^2$ break the
boson--fermion mass balance.

\subsection{Supermultiplet identification}

The sBootstrap identification maps the fermion component of each
meson superfield $M^i{}_j$ to a lepton, and the scalar component
to the physical meson. In the SUSY limit ($y \to 0$,
$\tilde m^2 \to 0$), each lepton is exactly degenerate with its
meson partner:

\begin{center}
\begin{tabular}{lccc}
\toprule
Multiplet & Fermion (lepton) & Scalar (meson) & SUSY-limit mass \\
\midrule
$\Phi_0$ (goldstino) & $e$ ($0.511\MeV$)
  & $\pi$ ($135\MeV$) & $\to 0$ (seed) \\
$\Phi_1$ (light) & $\mu$ ($105.7\MeV$)
  & $K$ ($494\MeV$) & $m_- = (2{-}\sqrt{3})\,m$ \\
$\Phi_2$ (heavy) & $\tau$ ($1{,}777\MeV$)
  & $D_s$ ($1{,}968\MeV$) & $m_+ = (2{+}\sqrt{3})\,m$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent
In the physical spectrum, the soft mass $\tilde m^2 = \fpi^2$
dominates the lightest multiplet---lifting $\pi$ far above~$e$
($m_\pi/m_e \approx 264$)---is comparable to the intermediate
one ($m_K/m_\mu \approx 4.7$), and negligible for the heaviest
($m_{D_s}/m_\tau \approx 1.11$).
This pattern---large fractional splitting for the lightest
generation, approximate degeneracy for the heaviest---is
characteristic of a universal soft mass $\tilde m^2 = \fpi^2$
applied to a hierarchical spectrum.
...
\midrule
Isospin seed: $m_s \approx (2{+}\sqrt{3})^2(m_u{+}m_d)$ & $m_s$ from $m_u{+}m_d$ & $\langle z_k^2\rangle \approx z_0^2$ on $(0,m_u{+}m_d,m_s)$; bloomed \\
...
Dual: $\langle\xi_k^2\rangle = \xi_0^2$ on $(d,s,b)$ & $m_d$ from $m_s, m_b$ & Seiberg seesaw \\
...
\midrule
\textbf{Remaining free:} & \multicolumn{2}{l}{$m_u,\, m_e,\, m_\mu,\, \theta_{23},\, \theta_{13},\, \delta_{\rm CP}$ (6 parameters)} \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Renormalisation scale.}
The quark masses appearing in the mass-charge triples are
$\overline{\rm MS}$ masses at the PDG conventional scales:
$m_{u,d,s}$ at $\mu = 2\GeV$, $m_c(m_c)$, $m_b(m_b)$, $m_t(m_t)$.
Running all six quarks to a common scale $\mu = 2\GeV$ degrades
the ratios sharply: $\langle z_k^2\rangle/z_0^2$ for $(-s,c,b)$
worsens from $1.2\%$ to $3.2\%$, and for $(c,b,t)$ from
$0.4\%$ to $6.2\%$.
In fact, the mass-charge ratio is \emph{exactly} invariant under
pure QCD running.  Under one-loop QCD, all quark masses run with
a universal anomalous dimension:
$m_i(\mu) = m_i(\mu_0)\,[\alpha_s(\mu)/\alpha_s(\mu_0)]^{\gamma_m}$.
The common factor cancels in the ratio:
\begin{equation}
  \frac{\sum c\,m_k^{(0)}}{(\sum \sqrt{c\,m_k^{(0)}})^2}
  = \frac{c\sum m_k^{(0)}}{c\,(\sum\sqrt{m_k^{(0)}})^2}
  = \frac{\sum m_k^{(0)}}{(\sum\sqrt{m_k^{(0)}})^2}\,,
\end{equation}
where $c = [\alpha_s(\mu)/\alpha_s(\mu_0)]^{2\gamma_m}$.
The mass-charge ratio depends only on mass \emph{ratios},
and QCD preserves all mass ratios.  Electroweak corrections
break this universality (up-type and down-type quarks have
different anomalous dimensions above $M_W$), but in the
confined sector where the identity is set, only QCD is active.
The isospin seed is a special case: $m_1 = 0$, $m_2 = m_u{+}m_d$, $m_3 = m_s$,
immune to running because all entries share the same anomalous dimension and
the ratio $m_s/(m_u{+}m_d)$ is an RG invariant.
The Gell-Mann--Oakes--Renner relation~\cite{GOR1968}
$m_\pi^2 f_\pi^2 = (m_u{+}m_d)\langle\bar qq\rangle$
makes this ratio a statement about physical hadron observables
($m_\pi$, $f_\pi$, $m_K$), explaining why the isospin seed
is scheme-independent.
None of the quark triples sits exactly at $\langle z_k^2\rangle = z_0^2$:
the bloom shifts each triple away from the seed,
by $0.27\%$ (isospin), $0.4\%$ $(c,b,t)$, and $1.2\%$ $(-s,c,b)$.
The lattice ratio $m_s/m_{ud} = 27.23 \pm 0.10$ (FLAG 2021)
resolves the isospin bloom at $6.3\sigma$ from the
exact seed value $2(2{+}\sqrt{3})^2 = 27.86$.
For the cross-threshold triples $(-s,c,b)$ and $(c,b,t)$,
the preferred convention---each quark at its own mass---is
natural in the ISS framework, where each meson $M_j$ has dynamics
at scale $\mu \sim m_j$.
...
$m_d$ & $4.6\MeV$ & $4.67 \pm 0.48$ ($0.1\sigma$)
  & Dual mass-charge identity \\
$m_u/m_d$ & $0.453$ & $0.46 \pm 0.05$ ($0.2\sigma$)
  & Self-consistency \\
...
All quark predictions follow from a single input
$m_u + m_d = 6.83\MeV$ (equivalently $m_\pi$ via GMOR);
the isospin seed gives $m_s$, the O'Raifeartaigh chain gives $m_c$,
the bion gives $m_b$, and the dual mass-charge identity gives $m_d$ (hence $m_u$).
Cross-threshold masses are $\overline{\rm MS}$ at $\mu = m_q$.
...
%======================================================================
\section{The Lagrangian from the viewpoint of hadronic superstrings}
\label{sec:strings}
%======================================================================

The composites of the sBootstrap --- mesons in the $\mathbf{24}$ and
diquarks in the $\mathbf{15} \oplus \overline{\mathbf{15}}$ --- fill
exactly the representations that appear when the adjoint of $SO(32)$ is
decomposed under $SU(5) \times SU(3)$.  The group $SO(32)$ is the gauge
group of Type~I open superstring theory, and this coincidence invites a
literal interpretation: the sBootstrap Lagrangian may be the low-energy
effective theory of a superstring at the hadronic scale.  Such a string
would not live at the Planck scale ($10^{19}\GeV$) but at the QCD scale
($\sim 1\GeV$) --- returning string theory to the hadronic setting in
which it was originally conceived~\cite{Veneziano1968, Nambu1970}.

This section develops that interpretation in detail.

\subsection{Open strings and Chan-Paton labels}

A relativistic string is a one-dimensional extended object whose
quantised vibrations produce particles.  An \emph{open} string has two
free endpoints; among its massless vibrational modes are a spin-1
particle (a gauge boson) and, in the supersymmetric case, a spin-1/2
partner.  The mass gap to the first excited string mode is set by the
string tension $T = 1/(2\pi\alpha')$; the \emph{string scale} is
$M_s = 1/\sqrt{\alpha'}$.

Each endpoint of an open string carries a discrete label
$i \in \{1,\ldots,N\}$, called a \emph{Chan-Paton
label}~\cite{ChanPaton}.  A string state $|i, j\rangle$ (one label per
endpoint) transforms under $U(N)$: the $N^2$ massless gauge bosons fill
the adjoint of $U(N)$.

In \emph{Type~I} string theory, the string is unoriented: the
orientation-reversal projection $\Omega$ identifies $|i,j\rangle$ with
$|j,i\rangle$.  This halves the gauge degrees of freedom and projects
$U(N)$ down to $O(N)$ or $Sp(N)$, depending on the sign of the
orientifold projection.  Consistency of the ten-dimensional quantum
theory --- specifically, cancellation of gauge and gravitational
anomalies via the Green--Schwarz mechanism~\cite{GreenSchwarz} ---
requires exactly
\begin{equation}
N = 32\,,\qquad G = SO(32)\,.
\end{equation}
The adjoint representation is the antisymmetric product of two
fundamentals:
\begin{equation}
  \mathbf{496} = \wedge^2(\mathbf{32})\,,
\end{equation}
containing $32 \times 31/2 = 496$ gauge bosons.

\subsection{The $SO(32)$ adjoint contains the sBootstrap}

The fundamental $\mathbf{32}$ of $SO(32)$ is a real representation;
it decomposes under the maximal subgroup $SU(16) \times U(1)_A$ as
\begin{equation}
  \mathbf{32} = \mathbf{16}_{+1} \oplus \overline{\mathbf{16}}_{-1}\,.
\end{equation}
The 16-dimensional complex representation of $SU(16)$ further
decomposes under $SU(5) \times SU(3) \times U(1)_B$ as
\begin{equation}\label{eq:16-decomp}
  \mathbf{16} = (\mathbf{5},\mathbf{3})_{+1}
    \oplus (\mathbf{1},\mathbf{1})_{-15}\,.
\end{equation}
The 15 entries $(\mathbf{5},\mathbf{3})$ are precisely the five quark
flavors $(u,d,s,c,b)$ in three colors $(r,g,b)$.  The remaining
singlet $(\mathbf{1},\mathbf{1})$ is the top quark, which does not
participate in the composite sector.  The identity
\begin{equation}
  5 \times 3 + 1 = 16
\end{equation}
is the Chan-Paton dimension of a single D-brane stack.

\paragraph{Adjoint decomposition.}
The adjoint $\mathbf{496} = \wedge^2(\mathbf{32})$ decomposes under
$SU(16) \times U(1)_A$ as
\begin{equation}
  \mathbf{496} = (\mathbf{255} \oplus \mathbf{1})_0
    \oplus \mathbf{120}_{+2} \oplus \overline{\mathbf{120}}_{-2}\,.
\end{equation}
The further decomposition under $SU(5) \times SU(3)$ is given in
Table~\ref{tab:496}.
...
$\overline{\mathbf{120}}$
 & $\boldsymbol{(\overline{15},3)}$ & $\boldsymbol{45}$ & $-2$ & $-2$
   & $\boldsymbol{\overline{\textbf{diquarks}}}$ \\
...
\bottomrule
\end{tabular}
\caption{Decomposition of the $SO(32)$ adjoint under
$SU(5)_{\rm flavour} \times SU(3)_{\rm colour}$.  The boldface
entries are the sBootstrap composites: mesons
$(\mathbf{24},\mathbf{1})$ and diquarks
$(\mathbf{15},\bar{\mathbf{3}}) \oplus
(\overline{\mathbf{15}},\mathbf{3})$, accounting for
$24 + 45 + 45 = 114$ of the 496 states.}
\label{tab:496}
\end{table}

\paragraph{Why the symmetric $\mathbf{15}$ appears.}
The key algebraic content of $\wedge^2(\mathbf{16})$ comes from
$\wedge^2[(\mathbf{5},\mathbf{3})]$ --- the antisymmetric product
of two quarks, each carrying flavour and colour indices.
The antisymmetric product of a tensor-product space obeys the identity
\begin{equation}\label{eq:wedge-identity}
  \wedge^2(V \otimes W)
  = \bigl[S^2(V) \otimes \wedge^2(W)\bigr]
  \oplus \bigl[\wedge^2(V) \otimes S^2(W)\bigr]\,.
\end{equation}
Overall antisymmetry of the composite index $(ia)$ --- one flavour
index $i$ and one colour index $a$ per quark --- forces a correlation:
symmetric in one factor implies antisymmetric in the other.
Setting $V = \mathbf{5}$ (flavour), $W = \mathbf{3}$ (colour):
\begin{equation}
  \wedge^2\bigl[(\mathbf{5},\mathbf{3})\bigr]
  = \underbrace{(\mathbf{15},\bar{\mathbf{3}})}_{\text{flavour-symmetric,
    colour-antisymmetric}}
  \oplus \underbrace{(\overline{\mathbf{10}},\mathbf{6})}_{\text{flavour-antisymmetric,
    colour-symmetric}}\,.
\end{equation}
The symmetric flavour $\mathbf{15} = S^2(\mathbf{5})$ is forced into
antisymmetric colour $\bar{\mathbf{3}} = \wedge^2(\mathbf{3})$; the
pairing is not a modeling choice but a consequence of the tensor-product
structure of open-string states.

\paragraph{Why not $E_8 \times E_8$.}
The heterotic string in ten dimensions admits two possible gauge groups:
$SO(32)$ or $E_8 \times E_8$.  The second is excluded by the
sBootstrap.  Under the chain $E_8 \supset SU(5) \times SU(5)$,
every $E_8$ representation decomposes into exterior powers
of $\mathbf{5}$ ($\mathbf{5}$, $\overline{\mathbf{10}} =
\wedge^2\mathbf{5}$, $\mathbf{24}$, \ldots).  The symmetric product
$S^2(\mathbf{5}) = \mathbf{15}$ never appears.
This is not a matter of choosing a different embedding: it is an
algebraic obstruction rooted in the structure of $E_8$.  In the
$SO(16) \times SO(16)$ subgroup of $SO(32)$, the obstruction is
equally sharp: no single $\mathbf{16}$ of $SO(16)$ can hold the
15-dimensional block $(\mathbf{5},\mathbf{3})$, so $SU(5)$ and
$SU(3)$ embed as separate direct summands whose $\wedge^2$ produces
only $\overline{\mathbf{10}}$, never $\mathbf{15}$.
The sBootstrap thus selects $SO(32)$ over $E_8 \times E_8$ as the
unique string-theoretic gauge group compatible with its composite
spectrum.

\subsection{Compactification: the $\mathbb{Z}_3$ orbifold and three
generations}

Type~I superstring theory is consistent in ten spacetime dimensions.
To make contact with four-dimensional physics, the six extra dimensions
must be compact.  The simplest compact space is a six-torus
$T^6 = (T^2)^3$, where each factor $T^2$ is a two-dimensional torus
defined by a lattice $\Lambda \subset \mathbb{C}$.

A torus alone preserves too much supersymmetry ($\mathcal{N}=4$ in
4d) and produces no chiral fermions.  Both problems are solved by an
\emph{orbifold}: the quotient of $T^6$ by a discrete symmetry
group~$G$ that acts simultaneously on the compact coordinates and on
the Chan-Paton labels.  The surviving states are $G$-invariant.

For the sBootstrap, the natural choice is $G = \mathbb{Z}_3$, acting
on the three complex extra coordinates as
\begin{equation}
  \theta:\; (z_1, z_2, z_3)
  \;\longrightarrow\;
...
\end{equation}
This is the unique $\mathbb{Z}_3$ action that preserves exactly
$\mathcal{N}=1$ SUSY in four dimensions (it embeds into $SU(3)$
holonomy, satisfying the Calabi--Yau condition).

\paragraph{Chan-Paton embedding and tadpole cancellation.}
The $\mathbb{Z}_3$ must also act on the 16 D9-brane Chan-Paton labels
(before orientifold images) via a matrix $\gamma_\theta$ in the
fundamental.  Two consistency conditions must be satisfied:
\begin{enumerate}
\item The orientifold $\Omega$ exchanges branes with eigenvalue
$\omega$ and those with $\omega^2 = \bar\omega$, so their
multiplicities must be equal: $n_1 = n_2$.
\item The \emph{twisted tadpole cancellation} condition requires
$\mathrm{Tr}(\gamma_\theta) = 1$ (in the absence of D5-branes).
\end{enumerate}

With $n_0$ branes at eigenvalue 1, and $n_1 = n_2$ branes at
eigenvalues $\omega, \omega^2$:
\begin{equation}
  \mathrm{Tr}(\gamma_\theta) = n_0 + n_1\omega + n_2\omega^2
  = n_0 + n_1(\omega + \omega^2) = n_0 - n_1\,,
\end{equation}
using $\omega + \omega^2 = -1$.  The conditions $n_0 + 2n_1 = 16$ and
$n_0 - n_1 = 1$ give the unique solution
\begin{equation}
  n_0 = 6\,,\qquad n_1 = n_2 = 5\,.
\end{equation}
The Chan-Paton matrix is therefore
\begin{equation}
  \gamma_\theta = \mathrm{diag}(\underbrace{\omega,\ldots,\omega}_{5},\;
    \underbrace{\omega^2,\ldots,\omega^2}_{5},\;
    \underbrace{1,\ldots,1}_{6})\,.
\end{equation}
The five $\omega$-branes and five $\omega^2$-branes are exchanged by
$\Omega$, forming an $SU(5)$ gauge factor.  The six $\omega^0$-branes
must be partitioned into $\Omega$-exchanged pairs; the partition
$[3,3]$ gives
\begin{equation}
  G = SU(5)_{\mathrm{flavour}} \times SU(3)_{\mathrm{colour}}
  \times U(1)^2\,.
\end{equation}
The partition $[3,3]$ is the \emph{unique} choice yielding $SU(3)$
as a direct gauge factor: $[6]$ gives $SO(6)$, $[4,2]$ gives
$SU(4) \times SU(2)$, $[2,2,2]$ gives $SU(2)^3$.

\paragraph{Three chiral generations.}
The chiral spectrum from the D9-brane sector in the $\mathbb{Z}_3$
orbifold is
\begin{equation}
  3 \times \bigl[(\mathbf{10},\mathbf{1})
  + (\bar{\mathbf{5}},\mathbf{3})
  + (\mathbf{5},\bar{\mathbf{3}})\bigr]\,,
\end{equation}
where the multiplicity~3 is the number of $\mathbb{Z}_3$ twisted
sectors, and the $\mathbf{10}$ is the antisymmetric two-index
representation of $SU(5)_f$.  This antisymmetric projection
($\varepsilon = +1$) is forced by the odd dimension of the charge-0
blocks ($n = 3$), which cannot support an Sp-type projection (Sp
requires even dimension).  Whether the symmetric $\mathbf{15}$ can be
obtained through a modified orientifold remains an open question
(Section~\ref{sec:open}).

\subsection{The string scale and extra dimensions}

In string theory, gauge interactions (open strings) are confined to
the D-branes, while gravity (closed strings) propagates in all ten
dimensions.  The effective four-dimensional Planck mass $M_{\rm Pl}$
depends on the total volume~$V_6$ of the compact space.

\paragraph{Derivation.}
In ten dimensions, Newton's constant is $G_{10} \sim g_s^2/M_s^8$.
In four dimensions after compactification on a volume~$V_6$,
Gauss's law gives $G_4 = G_{10}/V_6$, hence
\begin{equation}\label{eq:Mpl-Ms}
  M_{\rm Pl}^2 = \frac{1}{G_4}
  \;\sim\;
  \frac{M_s^{8}\,V_6}{g_s^2}\,.
\end{equation}
If $n$ of the six dimensions have a common radius~$R$ (the
``large'' dimensions) and the remaining $6 - n$ have the string
radius $R_s \sim 1/M_s$ (the ``small'' dimensions):
\begin{equation}
  V_6 \sim R^n \times M_s^{-(6-n)}\,,
  \qquad\text{so}\qquad
  M_{\rm Pl}^2 \;\sim\;
  \frac{M_s^{n+2}\,R^n}{g_s^2}\,.
\end{equation}
Solving for~$R$:
\begin{equation}\label{eq:R-extra}
  R \;\sim\;
  \left(\frac{g_s^2\,M_{\rm Pl}^2}{M_s^{n+2}}\right)^{\!1/n}\,.
\end{equation}

\paragraph{A hadronic string scale.}
In the sBootstrap, the dynamical scale is
$\Lambda_{\text{QCD}} \sim 300$~MeV.  Setting the string scale at
$M_s \sim 1$~GeV places the string at the \emph{hadronic} scale ---
historically, the setting in which string theory was
invented~\cite{Veneziano1968, Nambu1970} as a model for the Regge
trajectories of hadrons.

With $M_s = 1$~GeV and $g_s = 1$, equation~\eqref{eq:R-extra}
gives the extra-dimension radius as a function of~$n$.  Using
$M_{\rm Pl} = 1.22 \times 10^{19}$~GeV:
\begin{center}
\begin{tabular}{ccl}
\toprule
Large dims $n$ & $R$ & Status \\
\midrule
2 & $\sim 400$~m & excluded \\
3 & $\sim 0.2$~mm & excluded (short-range gravity tests) \\
...
\noindent
Current torsion-balance experiments~\cite{Adelberger2009} constrain
deviations from Newton's law at distances down to $\sim 40\;\mu$m.
The scenario with $n \geq 4$ large extra dimensions is experimentally
viable.  The estimates above assume $g_s \sim 1$;
a smaller coupling reduces~$R$.

Equivalently, three of the six compact dimensions could be at the
string scale and three large, giving
$R \sim 0.2$~mm --- already excluded.  With four or more large
dimensions the radii drop to the micrometre scale or below, safely
out of reach.

This is a low-scale string theory in the sense of
Arkani-Hamed--Dimopoulos--Dvali, but applied at $M_s \sim 1$~GeV
rather than $M_s \sim$~TeV.  The gauge hierarchy is not solved ---
it is \emph{inverted}: the puzzle is why gravity is so weak
compared to the hadronic scale.

\subsection{The $SU(3)$ root lattice and $gv/m = \sqrt{3}$}

The $\mathbb{Z}_3$ orbifold requires each $T^2$ to have a lattice
compatible with the three-fold rotational symmetry.  The unique such
lattice (up to scaling) is the $SU(3)$ root lattice, generated by
the simple roots
\begin{equation}
  e_1 = 1\,,\qquad
  e_2 = e^{i\pi/3} = \tfrac{1}{2} + i\tfrac{\sqrt{3}}{2}\,,
\end{equation}
in the complex plane.  The modular parameter of the resulting torus is
$\tau = e^{i\pi/3}$, and the area of the fundamental domain is
\begin{equation}
  A = \mathrm{Im}(\tau) = \frac{\sqrt{3}}{2}
  \qquad\text{(in units of $\alpha'$)}\,.
\end{equation}

The $\mathbb{Z}_3$ orbifold has three fixed points on each $T^2$,
located at
\begin{equation}
  z = 0,\qquad
  \tfrac{1}{3}(1 + \tau),\qquad
  \tfrac{1}{3}(2 + 2\tau)\,.
\end{equation}
These form an equilateral triangle whose area is
$\sqrt{3}/12$ (one-sixth of the fundamental domain).

The number $\sqrt{3}$ appears here in a purely geometric role: it is
the imaginary part of the modular parameter of the unique
$\mathbb{Z}_3$-compatible torus.  In the sBootstrap Lagrangian, the
pseudo-modulus is stabilised at $\langle X\rangle/\mu = \sqrt{3}$
(equivalently $gv/m = \sqrt{3}$).  Two suggestive connections
deserve mention:
\begin{itemize}
\item The K\"ahler modulus $T$ of each $T^2$ satisfies
  $\mathrm{Im}(T) = \sqrt{3}/2$, and
  $gv/m = \sqrt{3} = 2\,\mathrm{Im}(T)$.
\item The proper distance from $\langle X\rangle = 0$ to the
  K\"ahler pole at $\sqrt{3}\,\mu$ is
  $\sigma_{\max} = \sqrt{3}\,\pi\,m/(4g)$
  (Section~\ref{sec:lagrangian}), a finite extent
  that could correspond to the size of the fundamental domain
  in the compact space.
\end{itemize}

Whether these coincidences reflect a physical mechanism --- the
orbifold geometry fixing the pseudo-modulus VEV --- is an open
problem.  A concrete realisation would require solving the moduli
stabilisation problem on the $T^6/\mathbb{Z}_3$ orbifold with the
sBootstrap D-brane configuration.

\subsection{Fractional instantons and the bloom potential}

At each $\mathbb{Z}_3$ fixed point of $T^2$, the gauge bundle can
carry a \emph{fractional instanton}: an instanton configuration whose
topological charge is $1/3$ of the bulk instanton charge.  The
instanton action is correspondingly reduced:
\begin{equation}
  S_{\text{frac}} = \frac{S_0}{3}\,,
\end{equation}
where $S_0 = 8\pi^2/g_{\text{YM}}^2$ is the full instanton action.
The fractional instanton amplitude is $\sim e^{-S_0/3}$, parametrically
larger than the full instanton amplitude $\sim e^{-S_0}$.

The three fixed points on a single $T^2$ each contribute a fractional
instanton.  In the effective superpotential, the $\mathbb{Z}_3$ phases
of the twisted sectors produce contributions proportional to
$e^{2\pi i k/3}$ ($k = 0, 1, 2$).  The product over three
eigenvalues (one per colour, in the colour-flavour locked phase)
yields the determinant $\det M$, and the exact identity
\begin{equation}
  \prod_{k=1}^{3}\bigl[1 + \sqrt{2}\cos(\delta + 2\pi k/3)\bigr]
  = -\tfrac{1}{2} + \tfrac{\cos 3\delta}{\sqrt{2}}
\end{equation}
(proved in Section~\ref{sec:bloom}) reflects the $\mathbb{Z}_3$
averaging over fixed points.

The three-instanton operator in the sBootstrap Lagrangian,
\begin{equation}
  W_3 = c_3\,\frac{(\det M^{(3)})^3}{\Lambda^{18}}\,,
\end{equation}
has a natural interpretation as the product over the three copies of
$T^2$ in $T^6 = (T^2)^3$:
\begin{center}
\begin{tabular}{lcl}
\toprule
Orbifold structure & & Lagrangian term \\
\midrule
3 eigenvalues per $T^2$ & $\to$ & $\det M$ (product of 3) \\
3 copies of $T^2$ & $\to$ & $(\det M)^3$ (cube) \\
$3 \times 3 = 9$ fractional instantons & $\to$ &
  $\mathbb{Z}_9 \subset \mathbb{Z}_{18}$ symmetry \\
$\cos(3\delta)$ per $T^2$ & $\to$ &
  $\cos(9\delta)$ subdominant harmonic \\
\bottomrule
\end{tabular}
\end{center}

\noindent
The dominant $\cos(3\delta)$ harmonic is the $\mathbb{Z}_3$
harmonic of a single torus; the $\cos(9\delta)$ subdominant
(34\% of the $\cos(3\delta)$ amplitude) is the $\mathbb{Z}_9$
harmonic arising from the product over all three tori.  The
effective bloom potential inherits its harmonic structure from the
orbifold geometry.

\subsection{Self-referential structure}

The sBootstrap is self-referential in a precise sense.  The
Chan-Paton labels of the Type~I string are the quarks themselves:
\begin{equation}
  \underbrace{u,\, d,\, s,\, c,\, b}_{\text{5 flavours}}
  \;\times\;
  \underbrace{r,\, g,\, b}_{\text{3 colours}}
  \;+\;
  \underbrace{t}_{\text{singlet}}
  \;=\; 16 \text{ labels}\,.
\end{equation}
In Type~I, each D9-brane and its orientifold image form a pair:
16~branes $+$ 16~images $= 32$ Chan-Paton indices, giving $SO(32)$.
The adjoint of this $SO(32)$ decomposes to contain
$(\mathbf{24},\mathbf{1}) \oplus (\mathbf{15},\bar{\mathbf{3}})
\oplus (\overline{\mathbf{15}},\mathbf{3})$ --- exactly the mesons
and diquarks that those same quarks form as composites.

The logical circle is:
\begin{enumerate}
\item The quarks define the Chan-Paton labels ($5 \times 3 + 1 = 16$).
\item The labels define the gauge group ($SO(32)$).
\item The adjoint of $SO(32)$ contains the scalar composites
  of the quarks.
\item Those composites fill the supermultiplets of the
  $\mathcal{N}=1$ Lagrangian.
\end{enumerate}

\noindent
Whether this circularity is a deep consistency condition --- a
``bootstrap'' in the original sense of the word --- or a coincidence of
small numbers is the central interpretive question raised by the
$SO(32)$ embedding.
...
$SO(32)$ string state (Section~\ref{sec:strings}). However, the mixed anticommutator
...
\paragraph{Orientifold routes to the symmetric $\mathbf{15}$.}
The $\mathbb{Z}_3$ orbifold of Section~\ref{sec:strings}
produces the antisymmetric~$\mathbf{10}$ because the
$SU(5)$ Chan-Paton block has odd dimension ($n_1 = 5$),
which cannot support an Sp-type ($\varepsilon = -1$)
projection.  Three natural modifications fail to
circumvent this:
\begin{enumerate}
\item \emph{D5-branes.}  Adding D5-branes at fixed points
  shifts the twisted tadpole.  The only even-$n_1$ solution
  is $n_1 = 4$ (twisted tadpole~$=4$), giving $SU(4)$
  rather than $SU(5)$.
\item \emph{Discrete torsion.}  Flipping the twisted-sector
  sign reverses the tadpole condition to
  $n_0 - n_1 = -1$, giving $n_1 = 17/3$ --- not an integer.
\item \emph{Changing the gauge group.}  The constraint
  $n_0 + 2n_1 = 16$, $n_0 - n_1 = 1$ has the unique
  solution $n_1 = 5$; no other integer satisfies both.
\end{enumerate}
Two viable routes remain, both involving deformations away
from the strict orbifold point.  First, in \emph{magnetized
brane models} (intersecting branes on~$T^6$), the chiral
spectrum arises from intersection numbers $I_{aa'}$ of a
brane stack~$a$ with its orientifold image~$a'$.  The
symmetric~$\mathbf{15}$ appears when $a$ wraps cycles
intersecting $O^-$~planes, and three generations require
$I_{aa'} = 3$.  Second, \emph{local $O5^-$~planes} at
the $\mathbb{Z}_3$ fixed points can flip the projection
from antisymmetric to symmetric at the locus where the
$SU(5)$ matter lives, even though the global gauge group
remains~$SO(32)$.  Both mechanisms select a specific
region of the Type~I moduli space; the sBootstrap's
requirement of the symmetric~$\mathbf{15}$ is thus
a \emph{moduli constraint} on the string vacuum.
...
A natural candidate for a dynamical origin is the instanton structure of
SQCD: for $N_f = N_c = 3$, instantons break $U(1)_{R+2A} \to \mathbb{Z}_{18}$,
which contains $\mathbb{Z}_9$ as a subgroup. The three-instanton operator
$W_3 = c_3(\det M)^3/\Lambda^{18}$ generates a $\delta$-dependent
potential. To see the harmonic content, note the exact identity
\begin{equation}
  \prod_{k=1}^{3}\bigl[1 + \sqrt{2}\cos(\delta + 2\pi k/3)\bigr]
  = -\tfrac{1}{2} + \tfrac{\cos 3\delta}{\sqrt{2}}\,,
\end{equation}
so $\det M = z_0^6\,f(\delta)^2$ with
$f = -1/2 + \cos(3\delta)/\sqrt{2}$.
The potential $|W_3|^2 \propto f(\delta)^{12}$ contains all harmonics
$\cos(3n\delta)$ for $n = 0,\ldots,6$; the $\cos(9\delta)$ term
(the unique $\mathbb{Z}_9$ harmonic) carries only $34\%$ of the
amplitude of the dominant $\cos(3\delta)$ mode.
The effective potential therefore has six minima per $2\pi$ cycle,
not nine.

The bion K\"ahler potential $\delta K \sim |v_0 - 2v_0^{\rm seed}|^2$
constrains the overall scale $z_0$ (radial direction in the
Cartan plane), while the three-instanton $F$-term potential constrains
the angle~$\delta$.
For the physical $(-s,c,b)$ triple, the bloom angle
$\delta_{\rm phys} = 159^\circ$
lies $24^\circ$ past the seed and $21^\circ$ before the nearest
instanton well at $180^\circ$, at a \emph{maximum} of the
combined effective potential
$V_{\rm eff} = -|f|^{2p} + r\,(S-3)^2$
for all powers $p$ and positive $r$.
The physical point is therefore a \emph{saddle}:
stable in the radial direction (bion minimum), unstable in the
angular direction (instanton-driven).
This is the same metastability structure as the ISS vacuum itself---the
angular instability is slow and the configuration is long-lived.
The bloom angle remains an open problem;
it may be fixed by the overdetermination of the mass chain
rather than by a simple potential minimum.

\paragraph{Dual self-consistency.}
The Seiberg seesaw maps direct charges $\zch_k = \sqrt{m_k}$
to dual charges $\xi_k = 1/\sqrt{m_k}$. In terms of the
elementary symmetric polynomials $e_i$ of the~$\zch_k$,
the dual variance ratio is
$\langle\xi_k^2\rangle/\xi_0^2 = 1 - 2e_1 e_3/e_2^2$.
In the angular parametrisation, requiring
$\langle z_k^2\rangle = z_0^2$ and
$\langle\xi_k^2\rangle = \xi_0^2$ simultaneously reduces to
\begin{equation}
  \cos 3\delta = \frac{5\sqrt{2}}{8}\,,
\end{equation}
an isolated angle distinct from the seed ($\delta = 3\pi/4$).
At the seed itself the identity
$\langle z_k^2\rangle = z_0^2 \Leftrightarrow
\langle\xi_k^2\rangle = \xi_0^2$ is automatic (one mass vanishes).
For the physical down-type quarks $(d,s,b)$,
$\langle\xi_k^2\rangle/\xi_0^2 = 0.997$ ($0.22\%$ from unity)
while $\langle z_k^2\rangle/z_0^2 = 1.10$---far from unity.
The seesaw thus creates an independent near-identity condition
on the dual charges, constraining $m_d$ from $(m_s, m_b)$.

\subsection{Supermultiplet restoration}

The central open problem of the sBootstrap is to identify the
exact supermultiplet pairings: which meson is degenerate with
which lepton in the SUSY limit ($f \to 0$, soft terms off)?

A complete Lagrangian for the sBootstrap would have the property
that each SUSY-breaking parameter can be dialled continuously to
zero, and at each stage the spectrum reorganises into
progressively more degenerate fermion--scalar pairs.  At $f = 0$,
$\tilde{m}^2 = 0$, and bloom parameter $\varepsilon = 0$, the
spectrum collapses to three exact $\mathcal{N}=1$ chiral
multiplets at masses~$(0,\, m_-,\, m_+)$.  This ``restoration''
would be the definitive signature of the Miyazawa supersymmetry.

In the O'Raifeartaigh superpotential at $f = 0$, the fermion
and scalar mass matrices become degenerate: each fermion
eigenvalue~$m_k$ has a scalar partner with the same mass.
Turning $f$ back on splits each pair by the $B$-term~$\pm 2gf$,
and the founding relation $m_\pi^2 - m_\mu^2 \approx \fpi^2$
is precisely this SUSY-breaking mass splitting for one such pair.

Within the $N_f = 4$ ISS sector, the O'Raifeartaigh eigenvalue
structure groups quarks as $(u,d)$ at eigenvalue~$0$ and $(c,s)$
at eigenvalues~$(m_+, m_-)$ --- consistent with the standard
generations and with $|V_{ud}| \approx |V_{cs}| \approx 1$.
The Cabibbo angle is the rotation between the eigenvalue basis
and the mass eigenbasis within this $2\times 2$ block.
The bottom quark enters through the bion mechanism, with
$\sqrt{m_b} = 3\sqrt{m_s} + \sqrt{m_c}$ mixing the~$m_-$
and~$m_+$ eigenvalues.  The $V_{cb}$ element of the CKM matrix
measures this mixing: it arises because~$b$ is a coherent
superposition of ISS eigenstates, not a pure eigenvalue.
The cross-generation pairings $(t,s)$, $(u,b)$, $(c,d)$
mentioned in the sBootstrap literature refer to the
$SU(5)$ representation content, not the O'Raifeartaigh
eigenvalue grouping.

The mesino masses at the Seiberg vacuum ($\sim 10^{-3}$--$10^{-2}\MeV$,
with $\langle z_k^2\rangle/z_0^2 = 0.44$) are too small and too
degenerate to serve as lepton masses. This does not mean that
a separate confining sector is needed; rather, it means that the
lepton masses are the \emph{tree-level} fermion eigenvalues of
the O'Raifeartaigh mass matrix --- the same eigenvalues
$(0,\, m_-,\, m_+)$ that generate the quark spectrum --- transmitted
to the lepton sector through the Miyazawa superpartner identification.
A preliminary calculation reveals that the $B$-term splitting
is \emph{non-universal}: it acts only on the $\Phi_1$ direction
of the O'Raifeartaigh superpotential. The mass eigenstates
$\psi_\pm$ have different $\Phi_1$ content:
$|\langle\psi_-|\Phi_1\rangle|^2 = 0.067$ (muon eigenstate) versus
$|\langle\psi_+|\Phi_1\rangle|^2 = 0.933$ (tau eigenstate).
The founding relation $m_\pi^2 - m_\mu^2 \approx \fpi^2$ works
precisely because the muon eigenstate has very little $\Phi_1$
content, suppressing its splitting by a factor $\sim 1/14$ relative
to the tau.  Fixing the muon--pion splitting to $\fpi^2$ predicts
a tau meson partner $33\MeV$ above~$m_\tau$, i.e.\ at $\sim 1810\MeV$.
The $D^\pm$ at $1870\MeV$ is $93\MeV$ above~$m_\tau$, a factor~$2.8$
larger than predicted; the $m^2$~splitting
$m_D^2 - m_\tau^2 = 3.4 \times 10^5\MeV^2$ exceeds the predicted
$1.2 \times 10^5\MeV^2$ by a factor~$\sim 3$.
The apparent $3\%$ mass-level agreement is an artefact of the
dominance of~$m_\tau^2$; the splitting itself is only
$3.7\%$ of~$m_\tau^2$, so any meson within
$\sim 200\MeV$ of the tau would ``match'' at the $10\%$ level.
Identifying the true tau partner requires either closing this
factor-of-three gap with radiative corrections or finding a
hitherto unobserved $J^P = 0^-$ state near~$1810\MeV$.
The splitting ratio $\cos^2(\pi/12)/\sin^2(\pi/12) = (2+\sqrt{3})^2
\approx 13.93$ is the \emph{same} structural constant that controls
the O'Raifeartaigh fermion mass ratio $m_+/m_- = (2+\sqrt{3})^2$;
both are consequences of the mass-charge condition $t = \sqrt{3}$.

In the mass-charge parametrisation, the seed spectrum
$(0,\, m_-,\, m_+)$ sits at $\delta = 3\pi/4$ where the lightest
eigenvalue vanishes.  The bloom
$\Delta W = \varepsilon\,\Phi_0\Phi_2$ rotates~$\delta$ by
only $2.3^\circ$, creating the electron mass while shifting
the muon eigenstate \emph{down} by $16\%$ and the tau eigenstate
\emph{up} by $1\%$ relative to the seed values.
At the seed, $m_+/m_- = (2+\sqrt{3})^2 = 13.93$; at the
physical point, $m_\tau/m_\mu = 16.82$.  The entire
difference is the bloom correction.

\paragraph{The supercharge and the two zeros.}
The supercharge~$Q$ that maps each lepton to its meson partner
acts identically on quarks (mapping them to diquarks) and on
leptons (mapping them to mesons).  Its explicit action on the
three O'Raifeartaigh multiplets is
\[
  Q\lvert X\rangle = \lvert e\rangle\,,\qquad
  Q\lvert \Phi_-\rangle = \lvert \mu\rangle\,,\qquad
  Q\lvert \Phi_+\rangle = \lvert \tau\rangle\,,
\]
where $\lvert X\rangle$ is the pseudo-modulus (a flavour singlet,
not a standard meson), and $\lvert \Phi_\pm\rangle$ are the scalar
mass eigenstates identified with the pion and~$D$ meson respectively.

Two particles attempt to reach zero mass through independent
protection mechanisms.  The \emph{electron}, sitting in the
goldstino direction, has zero $\Phi_1$ content and therefore
zero $B$-term splitting; at the seed
$\delta = 3\pi/4$ its mass vanishes exactly.  The \emph{pion},
a pseudo-Goldstone boson, would be massless in the chiral limit
$m_u = m_d = 0$.  The electron succeeds:
$m_e = 0.511\MeV$ is a residue of the $2.3^\circ$ bloom.
The pion fails: the GMOR
relation $m_\pi^2 = (m_u{+}m_d)\,B_0$ lifts it to $140\MeV$.
The founding relation
\[
  m_\pi^2 - m_\mu^2 \;=\; \fpi^2
\]
links these two stories:
the left-hand side is the $B$-term splitting inside
the muon supermultiplet (SUSY breaking), while the right-hand
side is the chiral symmetry breaking scale.
The supercharge~$Q$ unifies them---the same operator that maps
$\mu \leftrightarrow \pi$ connects the SUSY-breaking mass
splitting to the chiral condensate.

The SUSY-breaking hierarchy among the three multiplets is
inverted relative to mass:
the tau--$D$ pair is only $11\%$ split in~$m^2$
(nearest to degeneracy),
the muon--pion pair is $74\%$ split,
and the electron--$X$ pair is maximally broken
(a $0.5\MeV$ fermion paired with a
$\sim 13\MeV$ Coleman--Weinberg scalar).
Heavier multiplets are \emph{more} supersymmetric.
The massive spectrum---not the seed with its zero---is the
structure that the meson sector displays:
all four charged pseudoscalar mesons
($\pi^\pm$, $K^\pm$, $D^\pm$, $D_s^\pm$) are massive, with
no near-massless member.  The electron has ``escaped''
into near-masslessness because its direction in field space is
the goldstino, orthogonal to the meson adjoint.
...
Yukawa identity $y_j\langle M_j\rangle = \sqrt{2}\,C/v_{\text{EW}}$
(derived in Section~\ref{sec:lagrangian})
...
\subsection{M-theory uplift and the top--neutrino duality}

The Type~I $SO(32)$ string theory of Section~\ref{sec:strings}
admits a lift to M-theory.  The route avoids the $E_8 \times E_8$
obstruction: Type~I on a circle is T-dual to Type~I$'$
(Type~IIA with O8-planes and D8-branes), and Type~IIA lifts to
M-theory on~$S^1$.  The path is
\begin{equation}
  \text{Type I}\;(SO(32))
  \;\xrightarrow{\;T\text{-dual}\;}
  \text{Type I}'
  \;\xrightarrow{\;g_s \to R_{11}\;}
  \text{M-theory}\,.
\end{equation}
At no stage does $E_8 \times E_8$ appear.

The electric--magnetic dualities that permeate the sBootstrap ---
the Seiberg seesaw $\langle M_j\rangle = C/m_j$, the dual
mass-charge identity $\xi_k \propto 1/\sqrt{m_k}$, the
self-duality of the seed --- are precisely the structures that
become geometric in M-theory, where S-duality of Type~IIB lifts
to the $SL(2,\mathbb{Z})$ modular symmetry of the M-theory torus
fiber.  A theory built entirely on Seiberg duality is a theory
whose natural home is eleven-dimensional.

A suggestive numerological structure supports this.  In $D = 11$
supergravity, the on-shell bosonic degrees of freedom are
\begin{equation}
  128 = \underbrace{44}_{\text{graviton}\; g_{MN}}
      + \underbrace{84}_{\text{3-form}\; C_{MNP}}\,,
\end{equation}
where $44 = 9 \times 10/2 - 1$ (traceless symmetric tensor of
$SO(9)$) and $84 = \binom{9}{3}$ (antisymmetric 3-index tensor).
The graviton count decomposes as
\begin{equation}
  44 = 32 + 12\,,
\end{equation}
where $32$ is the dimension of the $SO(32)$ fundamental ---
the Chan-Paton labels --- and $12 = r^2 + s^2 - 1 = 4N$ is the
neutral-composite count from the Diophantine bootstrap.

The ``$12$'' admits two dual interpretations corresponding to
the two particles that sit outside the main composite spectrum:

\begin{center}
\begin{tabular}{lll}
\toprule
Frame & The 12 is & Mass mechanism \\
\midrule
Electric & top quark (singlet, $5\!\times\!3+1$)
  & Dirac ($y_t \sim 1$) \\
Magnetic & neutrino (pseudo-modulus)
  & Majorana ($\sim (m/\Lambda_L)^6$) \\
\bottomrule
\end{tabular}
\end{center}

\noindent
In the electric frame the top acquires its Dirac mass while the
neutrino is massless; in the magnetic frame the neutrino acquires
a Majorana mass while the top is massless.  The two particles are
at opposite extremes of the mass spectrum ($m_t/m_\nu \sim 10^{12}$),
and their product $m_t \times m_\nu$ would be set by the
duality scale --- in M-theory, by the geometry of the
compact 7-manifold.

Whether this duality can be made precise --- and whether the
$G_2$-holonomy manifold required for $\mathcal{N}=1$ in four
dimensions from M-theory inherits the $\mathbb{Z}_3$ structure
of the Type~I orbifold --- is an open problem that connects the
sBootstrap to the deepest structures of string/M-theory.
...
From the isospin combination $m_u + m_d = 6.83\MeV$
(equivalently $m_\pi$ via GMOR), the Lagrangian predicts
$m_s = 95.1\MeV$ ($+1.8\%$),
$m_c = 1301\MeV$ ($+2.0\%$), $m_b = 4233\MeV$ ($+1.3\%$),
$m_t = 171.3 \pm 1.5\GeV$ ($-0.9\%$), and $m_d = 4.6\MeV$ ($-1.5\%$),
via the isospin seed, the O'Raifeartaigh mass ratio $(2{+}\sqrt{3})^2$,
the bion doubling with coefficient $N_c = 3$, the Yukawa eigenvalue
constraint $\langle z_k^2\rangle = z_0^2$, and the dual mass-charge identity
from the Seiberg seesaw.

\paragraph{4.\ The lepton sector from O'Raifeartaigh.}
The same O'Raifeartaigh algebraic structure applies to the
lepton sector --- as expected from the Miyazawa superpartner
identification of leptons with mesons --- predicting
...
\paragraph{6.\ Seven parameters eliminated.}
The isospin seed ($m_s$ from $m_u + m_d$), the O'Raifeartaigh
mass ratio ($m_c$ from $m_s$), the bion mass relation ($m_b$
from $m_s, m_c$), the Yukawa eigenvalue constraint ($m_t$ from
$m_c, m_b$), the dual mass-charge identity ($m_d$ from $m_s, m_b$),
the lepton mass-charge identity ($m_\tau$ from $m_e, m_\mu$),
and the GST relation ($\theta_{12}$ from $m_d, m_s$)
together determine seven of the Standard Model's thirteen
charged-fermion parameters. The remaining six are $m_u$, $m_e$,
$m_\mu$, $\theta_{23}$, $\theta_{13}$, and $\delta_{\rm CP}$.
The entire quark mass spectrum ($m_d$ through $m_t$) descends
from the single isospin combination $m_u + m_d$, with the
ratio $m_u/m_d = 0.453$ predicted by the self-consistency
of the coupled system ($\chi^2/\text{dof} = 0.12$,
$p = 0.97$ against PDG values).

\paragraph{7.\ The diquark obstruction and the hadronic superstring.}
...
dynamics). The $SO(32)$ adjoint of Type~I string theory provides a
natural UV completion containing both the $\mathbf{24}$ and
$\mathbf{15} \oplus \overline{\mathbf{15}}$; the alternative
$E_8 \times E_8$ is algebraically blocked. A $\mathbb{Z}_3$ orbifold
on the $SU(3)$ root lattice gives three generations and connects the
instanton structure to the bloom potential. The bloom mechanism
and a Lagrangian derivation of the Diophantine counting remain the
central open problems; the orientifold projection (which at the
$\mathbb{Z}_3$ orbifold point produces the antisymmetric~$\mathbf{10}$)
may be resolved by magnetized branes or local $O5^-$ planes that
flip the projection to the symmetric~$\mathbf{15}$.
...
G.~Veneziano, ``Construction of a crossing-symmetric, Regge-behaved
amplitude for linearly rising trajectories,''
Nuovo Cim.\ A \textbf{57}, 190 (1968).

\bibitem{Nambu1970}
Y.~Nambu, ``Quark model and the factorization of the Veneziano
amplitude,'' in \textit{Symmetries and Quark Models},
ed.\ R.~Chand (Gordon and Breach, 1970), p.~269.

\bibitem{ChanPaton}
J.~E.~Paton and H.-M.~Chan, ``Generalized Veneziano model with
isospin,'' Nucl.\ Phys.\ B \textbf{10}, 516 (1969).

\bibitem{GreenSchwarz}
M.~B.~Green and J.~H.~Schwarz, ``Anomaly cancellations in
supersymmetric $D=10$ gauge theory and superstring theory,''
Phys.\ Lett.\ B \textbf{149}, 117 (1984).

\bibitem{Adelberger2009}
E.~G.~Adelberger, J.~H.~Gundlach, B.~R.~Heckel, S.~Hoedl,
S.~Schlamminger, ``Torsion balance experiments: a low-energy
frontier of particle physics,'' Prog.\ Part.\ Nucl.\ Phys.\
\textbf{62}, 102 (2009).
