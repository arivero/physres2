...
parameter budget.
Section~\ref{sec:strings} interprets the Lagrangian as a low-scale
Type~I superstring on $T^6/\mathbb{Z}_3$, showing how the $SO(32)$
adjoint contains the sBootstrap composites and how the orbifold geometry
connects to the mass-charge structure.
...
$|V_{cb}| = |\sqrt{m_s/m_b} - e^{i\psi}\sqrt{m_c/m_t}| \geq 0.061$,
overshooting the PDG value $0.0422$ by $40\%$.
...
$V_{us}$ & $0.2236$ & $0.2243 \pm 0.0008$ ($0.9\sigma$)
  & GST relation \\
$m_\nu$ & $49.2$\,meV & $\lesssim 50$\,meV (atmospheric)
  & Top--neutrino duality: $\fpi^2/m_t$ \\
...
Numerically, the product is suggestive.  If the heaviest neutrino
mass is $m_\nu \approx 49\MeV$ (the atmospheric scale
$\sqrt{\Delta m^2_{\text{atm}}} \approx 50$\,meV), then
\begin{equation}
  \sqrt{m_t \times m_\nu}
  \;=\; \sqrt{172{,}760 \times 0.049}
  \;=\; 92.0\MeV
  \;\approx\; \fpi\,.
\end{equation}
This would give the duality relation
\begin{equation}
  m_t \times m_\nu \;=\; \fpi^2\,,
\end{equation}
identifying the SUSY-breaking order parameter~$\fpi$ as the
geometric mean of the top and neutrino masses.  The prediction
$m_\nu = \fpi^2/m_t^{\text{pole}} = 49.2$\,meV lies
squarely in the atmospheric neutrino window and is testable
by next-generation experiments (KATRIN, Project~8, JUNO).
The relation connects the two particles that sit outside
the main composite spectrum --- the top (Chan-Paton singlet)
and the neutrino (pseudo-modulus) --- through the scale
that splits their supermultiplets.
...
the orientifold projection (which currently produces the antisymmetric
$\mathbf{10}$ rather than the symmetric $\mathbf{15}$), and a
Lagrangian derivation of the Diophantine counting remain the central
open problems.
