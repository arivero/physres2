...
\emph{energy-balance condition} --- it states that the mean squared
...
vacuum, and proves the energy-balance identity $Q = 2/3$.
...
For the O'Raifeartaigh triple $(0,\, (2-\sqrt{3})\,m,\, (2+\sqrt{3})\,m)$
the numerator is $\sum m_k = 4m$ and the denominator is
$(\sqrt{2-\sqrt{3}} + \sqrt{2+\sqrt{3}})^2 = 4 + 2 = 6$ (since
$\sqrt{(2-\sqrt{3})(2+\sqrt{3})} = 1$), giving
$Q = 4m/6m = 2/3$ exactly. This is a theorem about the
O'Raifeartaigh superpotential at $gv/m = \sqrt{3}$, not an
...
Near the pole the CW contribution is negligible, and the effective
potential is minimized at $v_{\text{pole}}$.
...
No numerical tuning is required. The K\"ahler metric
...
$\det M - B\bar{B} = \Lambda^6$
provides a natural setting for the O'Raifeartaigh mechanism.
When quark masses are added via a tree-level superpotential
$W_{\text{tree}} = m_i M^i{}_i$, the diagonal meson equation requires
\begin{equation}
  \hat m_j + X\,\frac{\det M^{(3)}}{M^j{}_j} = 0
  \qquad(\text{no sum on } j)\,,
\end{equation}
which determines each diagonal entry of $M^{(3)}$ in terms of the
quark masses and $\Lambda_{\text{eff}}$. For a seed configuration in
which one quark mass vanishes ($m_u\to 0$), the corresponding $F$-term
remains nonzero so long as $\det M^{(3)}\neq 0$, which is guaranteed
by the quantum constraint. Supersymmetry is therefore spontaneously broken.

In the ISS magnetic dual, the O'Raifeartaigh parameters map as
$g \to h$, $m \to h\mu$, $v \to \langle X\rangle$, so the
energy-balance condition $gv/m = \sqrt{3}$ becomes
...
the $Q = 2/3$ surface. However, the one-loop soft-mass RG
...
$Q \to 1/3$---the \emph{opposite} extreme. The algebraic
preservation condition on the $Q = 2/3$
surface is incompatible with any uniform attractor flow
...
on the superpotential---as in the O'Raifeartaigh realization, where
$gv = \sqrt{3}\,m$ produces the exact seed. The condition is set at the
scale of SUSY breaking and then preserved by QCD-invariant mass ratios.
...
at which the energy-balance condition on \emph{masses} is equivalent
to the same condition on \emph{Yukawa couplings} for mixed
...
\textbf{Scheme dependence caveat.} The nominal pull uses the
...
The prediction is therefore more properly stated as
...
The energy-balance condition $Q = 2/3$ is satisfied exactly at the seed.
...
energy-balance condition applied to the charged lepton masses gives:
\begin{equation}
...
The Gatto--Sartori--Tonin relation~\cite{GST1968} gives the leading-order
mixing angle from the down-sector mass hierarchy:
\begin{equation}
...
Yukawa: $Q(c,b,t) = 2/3$ & $m_t$ from $m_c, m_b$ & $W_{\text{Yukawa}}$ at $\tan\beta = 1$ \\
Energy balance: $Q(e,\mu,\tau) = 2/3$ & $m_\tau$ from $m_e, m_\mu$ & $W_{\text{lepton}}$: O'Raifeartaigh \\
...
The energy-balance condition constrains but does not uniquely fix the
...
$gv/m = \sqrt{3}$ but leaves $\delta$ undetermined. Explicit R-breaking
perturbations push $Q$ away from $2/3$ at any nonzero strength, making
the seed an unstable fixed point. The bloom must therefore be
nonperturbative.
...
and too degenerate ($Q = 0.48$, not $2/3$) to reproduce lepton masses.
...
The adjoint $\mathbf{496} = \wedge^2(\mathbf{32})$ contains
the sBootstrap representations:
$\mathbf{24}$ in the neutral sector and
$\mathbf{15} \oplus \overline{\mathbf{15}}$ in the matter
sector. The remaining 442 states must be
projected out or made massive; no projection mechanism has been
identified. The alternative anomaly-free gauge group
$E_8 \times E_8$ is blocked because every decomposition of $E_8$
under $SU(5)$ produces only the antisymmetric $\mathbf{10}$, never
the symmetric $\mathbf{15}$.
...
energy-balance condition as a theorem.}
...
with $Q = 2/3$ exactly. The energy-balance condition is a
boundary condition on the superpotential, not an RG attractor
(one-loop RG drives $Q \to 1/3$, the opposite extreme).
...
Since all quark mass triples satisfying $Q \approx 2/3$
obligatorily mix up-type and down-type quarks, the energy-balance
condition on masses equals the condition on Yukawa couplings
...
eigenvalue constraint, the lepton energy-balance condition, and
...
The empirical evidence for the energy-balance condition---the
...
\bibitem{companion}
A.~Rivero, ``The energy-balance condition: empirical evidence for
$Q = 2/3$ in fermion mass triples,'' in preparation.
