...
The pseudo-modulus $L_2$ is massless at tree level and couples to the
rest of the spectrum only through the $s$-confining operator
$L_1 L_2 L_3/\Lambda_L^3$. Its Coleman--Weinberg mass is therefore
suppressed by $(m/\Lambda_L)^6$:
$$
  m_{L_2} \sim \frac{g^2 m}{4\pi}
    \left(\frac{m}{\Lambda_L}\right)^{\!6}\,.
$$
With the O'Raifeartaigh scale $m \sim z_0^2 \approx 314\MeV$ and
$g \sim O(1)$, a confinement scale $\Lambda_L \sim 9$--$12\GeV$
places $m_{L_2}$ in the atmospheric neutrino window $9$--$50$\,meV.
No large hierarchy is needed: the suppression is a power law, not
an exponential.
Whether this pseudo-modulus can be identified with a
physical neutrino depends on its quantum numbers under the
electroweak group---a question that requires specifying the embedding
of SU(2)$_{\rm conf}$ into the full gauge structure.

\paragraph{Large neutrino mixing from democratic confinement.}
The $s$-confining superpotential $W_{\rm dyn} = L_1 L_2 L_3 / \Lambda_L^3$
treats all three composites democratically: it is invariant under
the full permutation group~$S_3$.  The O'Raifeartaigh deformation
breaks this symmetry, but the dynamical superpotential remains
the dominant coupling of the pseudo-modulus~$L_2$ to the rest of
the spectrum.  If $L_2$ is identified with a neutrino, its
flavour couplings inherit the $S_3$ structure, which is the
natural source of large mixing angles.

This contrasts sharply with the quark sector.  There, the Seiberg
seesaw $\langle M_j\rangle = C/m_j$ creates a hierarchical
texture (the Fritzsch matrix) that produces small CKM angles
via $\sin\theta_C = \sqrt{m_d/m_s}$.  The same Lagrangian
thus explains both small quark mixing and large lepton mixing
through two different confinement mechanisms:
\begin{center}
\begin{tabular}{lll}
\toprule
Sector & Confinement & Mixing \\
\midrule
Quarks (SU(3)) & Seiberg seesaw (hierarchical) & small CKM \\
Leptons (SU(2)) & $s$-confinement (democratic) & large PMNS \\
\bottomrule
\end{tabular}
\end{center}
