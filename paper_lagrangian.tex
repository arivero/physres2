...
(v)~a mechanism selecting the lepton mass-spectrum phase
(the three-instanton operator $(\det M^{(3)})^3/\Lambda_{\text{eff}}^{18}$
generates a $\cos(9\delta)$ potential, but numerical evaluation shows
that leptons do not sit at its minima; the observed phase satisfies
$\delta \bmod 2\pi/3 \approx 2/9\;\text{rad}$, not $2\pi/9$,
suggesting a different selection mechanism).
...
$(\det M)^3/\Lambda^{18}$ generates a $\cos(9\delta)$ potential.
However, numerical evaluation shows that the observed lepton phase
($\delta \bmod 2\pi/3 \approx 0.2222 \approx 2/9\;\text{rad}$, confirmed
to 31~ppm and $0.83\sigma$ in~$m_\tau$) does \emph{not} coincide with
a $\mathbb{Z}_9$ minimum ($\delta = n \cdot 2\pi/9$); the offset
$9\delta \bmod 2\pi \approx 2.0$~rad is $32\%$ of a full cycle.
The ``$2/9$ radian'' coincidence involves a transcendental
$1/\pi$ factor and awaits an explanation.
