...
SUSY generator and by the $SO(32)$ decomposition. The ``wrong'' 
Pauli sign of the symmetric diquark is a compositeness indicator: 
the diquark is an extended object, not a point particle.
...
\subsection{The Pauli sign as a compositeness indicator}
\label{subsec:pauli}

The requirement of the symmetric $\mathbf{15}$ conflicts with a 
naive application of Fermi statistics. For spin-0, $L=0$ diquarks 
in the color $\bar{3}$ (antisymmetric) channel:

\begin{center}
\begin{tabular}{lccc}
\toprule
Property & Symmetry & Factor & \\
\midrule
Color $\bar{3}$ & Antisymmetric & $-1$ & \\
Spin 0 & Antisymmetric & $-1$ & \\
Spatial $L=0$ & Symmetric & $+1$ & \\
\midrule
Non-flavor product & Symmetric & $+1$ & \\
For overall A: & & & flavor must be \textbf{A} $= \overline{\mathbf{10}}$ \\
\bottomrule
\end{tabular}
\end{center}

Pauli statistics for point-like diquarks requires 
\emph{antisymmetric} flavor---the $\overline{\mathbf{10}}$. 
The sBootstrap SUSY generator and the $SO(32)$ embedding both 
demand the \emph{symmetric} $\mathbf{15}$. This is a genuine 
conflict for point particles.

The resolution lies in the contrast between the two SUSY 
directions. When the generator acts on leptons, it produces 
\emph{mesons}---and these work perfectly: $q\bar{q}$ involves 
distinguishable constituents, no Pauli constraint applies, and 
the meson spectrum is reproduced without tension. When the 
generator acts on quarks, it produces \emph{diquarks} in the 
``wrong'' representation. The wrong Pauli sign is not a bug but 
a \emph{compositeness indicator}: it tells us that the diquark 
is an extended object, not a point particle. The two quarks inside 
the diquark are spatially separated, so the $L=0$ assumption 
that enters the Pauli argument breaks down. The diquark is a 
bound state whose internal structure defeats the point-particle 
statistics, much as the deuteron (two fermions, spin 1) exists 
in an $L \neq 0$ state that accommodates both its spin and its 
antisymmetry.

This asymmetry between the lepton $\to$ meson and quark $\to$ 
diquark directions is a central structural feature of the 
sBootstrap: the hadronic SUSY maps cleanly onto the meson sector 
(where $q\bar{q}$ pairs are ``free'') but encounters resistance 
in the diquark sector (where $qq$ pairs must negotiate Fermi 
statistics). The resistance manifests as spatial extension---the 
diquark is necessarily a composite object with internal orbital 
structure, not a simple scalar.
...
$SO(32) \supset SU(5) \times \ldots$, the 496-dimensional adjoint 
decomposes to include the 
$\mathbf{1} + \mathbf{24}$ (mesons/sleptons) and the 
$\mathbf{15} + \overline{\mathbf{15}}$ (diquarks/squarks).

This is suggestive: the same group structure that gives anomaly-free 
string compactifications produces the symmetric diquark 
representation demanded by the SUSY generator. The $SO(32)$ embedding 
may provide the UV completion in which the compositeness of diquarks 
(and hence the resolution of the Pauli sign) is made explicit: in 
the string picture, diquarks are open-string states whose symmetry 
is determined by the gauge group, not by point-particle statistics.
...
The sBootstrap requires symmetric $\mathbf{15}$ diquarks; Pauli 
statistics for point-like spin-0, $L=0$, color $\bar{3}$ diquarks 
requires antisymmetric flavor ($\overline{\mathbf{10}}$). The 
compositeness interpretation (\S\ref{subsec:pauli}) needs 
quantitative support: what is the spatial extent of the diquark, 
and can the $L \neq 0$ structure be computed from QCD? The exotic 
$+4/3$-charged diquarks predicted by the $\mathbf{15}$ are a 
...
generator acting on quarks and by the $SO(32)$ decomposition. The 
conflict with Pauli statistics for point particles is a 
compositeness indicator: the diquark is an extended object whose 
internal spatial structure defeats the point-particle statistics 
argument.
