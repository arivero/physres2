...
how the three types of SUSY breaking (F-term, D-term, nonlinear 
Volkov--Akulov) may map onto the three-tuple structure.
\end{abstract}
...
SUSY-breaking order parameter. In this paper we develop two themes 
...
A historical note: Harari and a collaborator observed that $(0, d, s)$ 
was approximately a Koide triple, at a time when the up quark mass was 
...
been lifted (to $m_e$, or the sign-flipped strange/pion mass), and a 
new heavy member ($b$ quark, $B$ meson) has appeared. The full triple 
also satisfies $Q \approx 2/3$ but at a larger scale $v_0$.
\end{itemize}
...
deviating by $0.42\%$. Its vacuum energy $v_0^2 \approx 22.8\GeV$ 
...
The SUSY-breaking dynamics must accomplish three things simultaneously:
generate the seed-to-bloom transition for each tuple, preserve the 
energy-balance condition $\langle z_k^2\rangle = z_0^2$ through the 
breaking, and produce the correct splitting between meson and lepton 
masses at scale $f_\pi$. In this section we develop the framework in 
detail, connecting the three standard SUSY-breaking mechanisms to the 
...
The structural parallel with the seed triples is precise:

\begin{center}
\begin{tabular}{lll}
\toprule
O'Raifeartaigh & Seed triple & After breaking \\
\midrule
$\Phi_0$ (massless) & 0 member & Lifted to $\zch < 0$ \\
$\Phi_1$ (mass $m$) & $\zch_a$ (light) & Shifted \\
$\Phi_2$ (mass $m$) & $\zch_b$ (heavy) & Shifted \\
\midrule
$f = \langle F\rangle$ & --- & $f_\pi$ \\
\bottomrule
\end{tabular}
\end{center}
...
as the Goldstino coupling with $f_{\text{VA}} = f_\pi$. The two 
descriptions are related by:
\begin{equation}
f_{\text{VA}}^2 = \langle F\rangle = \frac{|\qqbar|}{f_\pi}\,,
\end{equation}
consistent with the GOR relation $m_\pi^2 f_\pi^2 = 
-(m_u + m_d)\qqbar$.
...
\subsection{Mapping the three mechanisms to the three tuples}
\label{subsec:mapping}

The three-tuple structure suggests a layered breaking:

\paragraph{Layer 1: F-term (chiral condensate, scale $\sim f_\pi$).}
...
The $(c,b,t)$ triple at $v_0^2 \approx 22.8\GeV$ operates far 
...
This layered structure has a natural hierarchy:
\begin{equation}
\underbrace{f_\pi \sim 92\MeV}_{\text{F-term, chiral}} 
\;\ll\; 
\underbrace{\xi^{1/2} \sim 35\MeV}_{\text{D-term, EM}} 
\;\ll\; 
\underbrace{v_0^{(cbt)} \sim 151\MeV^{1/2}}_{\text{heavy sector}}\,,
\end{equation}
where the scales refer to different physical quantities (the 
comparison is schematic). The F-term dominates the boson--fermion 
splitting; the D-term provides electromagnetic fine structure; the 
heavy sector is governed by its own dynamics.
...
The ISS mechanism (Intriligator--Seiberg--Shih \cite{ISS2006}) makes 
this precise: for $N_f$ slightly above $N_c$, SQCD has metastable 
SUSY-breaking vacua where the rank condition prevents all $F$-terms 
from vanishing. The sBootstrap case $N_f = N_c = 3$ is at the 
boundary of the ISS window, suggesting that the SUSY breaking may 
be long-lived but not absolutely stable---consistent with the 
approximate nature of the Koide relations.
...
in a single $3\times 3$ meson matrix}. The entries of $M^i{}_j$ are:
...
When heavier quarks ($c$, $b$) are included, the matrix extends to 
$5\times 5$ (or effectively $5\times 5$ minus the top row/column). 
The Koide triples are specific \emph{slices} through this matrix:

\begin{itemize}
\item The lepton triple $(e, \mu, \tau)$ corresponds to the 
fermionic components of specific matrix entries.
\item The meson seed $(0, \pi, D_s)$ involves the $(u\bar{d})$, 
$(c\bar{s})$ entries and a zero.
\item The full meson triple $(-\pi, D_s, B)$ involves 
$(u\bar{d})$, $(c\bar{s})$, $(u\bar{b})$.
\end{itemize}

The joining of the three supermultiplets is the central unsolved 
problem: how does a single $5\times 5$ (or $3\times 3$) meson 
matrix, subject to the Seiberg constraint, produce \emph{three} 
Koide triples with different $v_0$ values, each satisfying 
$\langle z_k^2\rangle = z_0^2$?
...
flows toward the Koide surface $Q = 2/3$. In the renormalization 
group language, the Koide condition would be a fixed point of the 
RG flow for the soft SUSY-breaking parameters.

This is plausible if the breaking dynamics has an extremum (minimum 
of effective potential, or maximum of entropy) at the equipartition 
point $\sigma = \mu$. The condition $\langle z_k^2\rangle = z_0^2$ 
says the variance of the charges equals their squared mean---this 
is the maximum-entropy distribution for three positive-definite 
quantities with a given mean, subject to a log-normal prior. If 
the SUSY-breaking effective potential favors ``democratic'' 
distributions of mass among the multiplet members, the Koide 
condition would emerge as a thermodynamic consequence.

The fact that the condition holds both for fundamental fermions 
(leptons, quarks) and composite bosons (mesons) supports the 
attractor interpretation: it is not specific to one sector but 
operates universally wherever three related masses coexist.

\subsection{What the $(c,b,t)$ triple tells us}

The $(c,b,t)$ triple ($Q = 0.6695$, $v_0^2 = 22.8\GeV$) has 
...
If interpretation (2) is correct, the sBootstrap has two breaking 
scales:
\begin{equation}
...
\begin{equation}
v_0 = 17.72, \quad z_e = -17.00, \quad 
\zch_e = 17.72 - 17.00 = 0.72\MeV^{1/2}\,.
\end{equation}
A 96\% cancellation. The mass $m_e = 0.72^2 = 0.52\MeV$ is the 
...
\paragraph{3.\ Three tuples require three breaking structures.}
F-term breaking (via the massless seed member) generates the light 
sector. D-term breaking ($U(1)_{\text{em}}$ FI parameter) provides 
electromagnetic splittings. Volkov--Akulov nonlinear realization 
sets the overall scale $f_{\text{VA}} = f_\pi$. The heavy triple 
