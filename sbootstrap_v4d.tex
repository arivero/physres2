...
Thus the sBootstrap representations
$\mathbf{24} \oplus \mathbf{15} \oplus \overline{\mathbf{15}}$
are exactly the $SU(5)$ content of the $SO(32)$ adjoint after
restricting to the neutral sector.  However, $496 - 226 = 270$
additional states with non-zero $U(1)$ charges must be projected
out.  The mechanism for this projection---whether by an orbifold,
...
spins ($s = 1/2$ maps to the $SU(2)_L$ doublet structure of $W$,
$s = 1$ to the triplet).  Then
\begin{equation}
R = 1 - \frac{x_+(1/2)}{x_+(1)}
  = \frac{(\sqrt{19} - 3)(\sqrt{19} - \sqrt{3}\,)}{16}
  \;\approx\; 0.2231014\ldots
\end{equation}
...
($m_b \gg m_s, m_c$). The bloom is far more dramatic than a 
...
and the fluctuations $z_k$ encode the deviations. If the 
energy-balance condition $\langle z_k^2\rangle = z_0^2$ is a 
dynamical attractor (as suggested in \S\ref{subsec:fixedpoint}), 
the $z_k$ may evolve under a flow in which the \emph{ratio} 
$z_a/z_b$ approaches $2 - \sqrt{3}$ at the fixed point while 
the individual masses (and their QCD-invariant ratios) remain 
unchanged. In this picture, the 1.2\% deviation measures how 
far the quark sector is from the attractor.
...
overlapping triples must agree on $m_c$ and $m_b$. The light quark masses
$m_u$, $m_d$ are not constrained by any Koide triple; they enter only
through the GOR relation for $m_\pi$.
...
The adjoint $\mathbf{496}$ decomposes into a 226-dimensional
neutral sector (containing the sBootstrap representations
$\mathbf{24} \oplus \mathbf{15} \oplus \overline{\mathbf{15}}$)
and 270 charged states that must be projected out or made massive
in a realistic model. No projection mechanism has been identified.
