...
triple doubles the vacuum charge, predicting
...
potential, superpotential (Seiberg constraint plus Yukawa),
$D$-terms, and soft breaking from a spurion with
$\langle F\rangle = f_\pi^2$---and show that the seed Koide
fixes the magnetic Yukawa coupling ($h = \sqrt{3}$) while the
$v_0$-doubling, being non-holomorphic, must originate in the
K\"ahler sector. After these constraints, only two to three
...
$\sin^2\theta_W = 0.22310$ to $0.13\sigma$.
...
this paper, quark masses follow PDG conventions: 
$m_s(2\GeV) = 93.4\MeV$, $m_c(m_c) = 1.27\GeV$, 
$m_b(m_b) = 4.18\GeV$, and the top pole mass 
$m_t = 172.76\GeV$. This is a mixed convention: the light 
quark is evaluated at 2 GeV while heavier quarks are 
evaluated at their own scale. Since QCD mass ratios are 
RG invariants (the anomalous dimension $\gamma_m$ is 
flavor-universal at leading order), the Koide ratio $Q$ is 
scheme-independent. However, the seed ratio 
$\sqrt{m_s}/\sqrt{m_c}$ as computed here uses masses at 
\emph{different} scales, giving $0.271$; at a common scale 
the ratio is $\sim 0.29$, further from $2 - \sqrt{3} = 0.268$. 
The meson seed, using unambiguous physical (pole) masses, 
is free of this scheme issue and provides the cleaner test.}
\begin{equation}
...
\paragraph{The vacuum-charge doubling.}
...
$4{,}180 \pm 30\MeV$ to $0.1\sigma$. This relation is
...
seed value---so these two constraints slightly overconstrain
the system.
...
Eleven pseudoscalar mesons $(\pi, K, \eta, \eta', D, D_s, B, B_s,
B_c, \eta_c, \eta_b)$ yield $\binom{11}{3} = 165$ triples, times
...
\end{equation}
where we have written $Q$ in the ``sum-of-roots-squared over
sum-of-masses'' convention ($Q = 3/2$ corresponds to the
energy-balance condition $Q_{\text{prev}} = 2/3$ of
\S\ref{sec:koide_algebra}).\footnote{The two conventions are related
by $Q_{\text{here}} = 1/Q_{\text{prev}}$. The value $3/2$ and the
value $2/3$ encode the same condition; we use $3/2$ in this section
because the literature on the Koide formula most commonly quotes
$Q = 2/3$ with the reciprocal normalization
$Q = \sum m / (\sum\sqrt{m})^2$, and we wish to avoid confusion with
that sign convention. Explicitly:
$Q_{\text{here}} = (\sum\sqrt{m})^2 / \sum m = 3/2$, while the
convention in Sec.~\ref{sec:koide_algebra} is
$Q_{\text{prev}} = \sum m / (\sum\sqrt{m})^2 = 2/3$.}
...
\end{equation}
The scale $M_0 \approx 314\MeV$ is the vacuum energy $v_0^2$ of
...
\begin{equation}
\delta_0 \bmod \frac{2\pi}{3} = 0.22223\;\text{rad}\,.
\end{equation}
This is strikingly close to the fraction $2/9 = 0.22222\ldots$:
\begin{equation}
\delta_0 \bmod \frac{2\pi}{3} - \frac{2}{9} = 7.8 \times 10^{-6}\,,
\end{equation}
a residual of $\mathbf{35}$~\textbf{ppm} relative to $2/9$.
(A figure of 7~ppm has appeared in the literature, but that uses the
unreduced phase $\delta_0$ as the denominator; the relevant
denominator is the value being matched, $2/9$.)
...
on $[0, 2\pi/3)$) falls within $7.8 \times 10^{-6}$ of \emph{any}
...
significance is $\mathbf{4.3\sigma}$. The $2/9$ proximity is the
strongest single numerical coincidence in the Koide system.
...
$x = M^2/m^2$ for a mass scale $m$ to be determined.  This is a
quadratic in $x$ (quartic in $M/m$).  Its solutions are
\begin{equation}
...
freedom.  The ratio derived below is therefore a
...
definition $\sin^2\!\theta_W = 1 - M_W^2/M_Z^2 = 0.22306$,
...
K\"ahler potential receives monopole--anti-monopole cross terms of the
form $|\!\sum\sqrt{m_k}|^2$, which are exactly the $|v_0|^2$ terms
needed for $v_0$-doubling. The principal caveat is that this analysis
...
\paragraph{The magnetic Yukawa coupling.}
...
quarks $q$, $\tilde q$ and the meson matrix $M$. The seed Koide
condition $gv/m = \sqrt{3}$ of the O'Raifeartaigh model translates
into $h = \sqrt{3}$: the magnetic Yukawa is fixed by requiring the
fermionic mass spectrum of the meson matrix to be an exact Koide seed.
This is a single algebraic constraint on the parameters of the magnetic
theory, reducing one coupling from free to determined.
...
This ``dual Koide'' on $(d,s,b)$ connects the UV mass hierarchy to
the IR meson VEVs: the Seiberg constraint maps the down-type quark
masses into a near-Koide spectrum of meson condensates.
...
flow. The vacuum energy $v_0 = z_0^2$ sets the common scale,
...
superfield. Below the confinement scale the theory is described by $M$,
together with baryon superfields $B$ and $\bar B$, subject to the
quantum-modified constraint~\eqref{eq:seiberg}:
%
\begin{equation*}
...
\begin{equation*}
  W_{\text{eff}}
  = \operatorname{Tr}(\hat m\, M)
  + X\!\left(\det M - B\bar B - \Lambda^{6}\right).
\end{equation*}
...
$\det M = \Lambda^6$. The diagonal meson equation then requires
%
\begin{equation*}
  \hat m_j + X\,\frac{\det M}{M^j{}_j} = 0
  \qquad(\text{no sum on } j)\,,
\end{equation*}
%
which determines each diagonal entry of $M$ in terms of the quark masses and
$\Lambda$. For a seed configuration in which one quark mass vanishes---say
$m_u\to 0$---the corresponding $F$-term
$F_{M^1{}_1}=X\,(\det M)(M^{-1})_1{}^1$ remains nonzero so long as
$\det M\neq 0$, which is guaranteed by the quantum constraint. Supersymmetry
is therefore spontaneously broken, with order parameter
%
\begin{equation*}
  \langle F \rangle \;\sim\; \fpi\,.
\end{equation*}
...
\end{align*}
%
where $f^2\approx \fpi^2$. The mass splitting is universal across the
multiplet:
%
\begin{equation*}
...
\end{equation*}
%
which is precisely the Volkov--Akulov relation~\eqref{eq:pimu}.
...
($c$, $b$), originate instead from electroweak symmetry breaking. Introducing
a Higgs doublet $H$, one writes a Yukawa superpotential
%
\begin{equation*}
  W_{\text{Yukawa}} = y_{ij}\, H\, Q^i\, \bar Q_j\,,
\end{equation*}
%
which is relevant only for quarks whose masses are dominated by the
...
Yukawa matrix $Y = \operatorname{diag}(y_c,y_b,y_t)$ with
$y_i = \sqrt{2}\,m_i/v_{\text{EW}}$. Since $Q$ is scale-invariant,
...
are the diagonal Gell-Mann matrices and $\hat n$ is a unit vector
in the Cartan plane determined by $\delta$. The Koide condition is
therefore a constraint on the \emph{direction} of the Yukawa
matrix in flavor space: the amplitude $\sqrt{6}/2$ is fixed, only
...
\end{equation*}
%
with K\"ahler potential (canonical),
%
\begin{equation*}
  K = \operatorname{Tr}(M^\dagger M) + |X|^2 + |B|^2 + |\bar B|^2
    + |H|^2\,,
\end{equation*}
%
...
\begin{equation*}
  W = \underbrace{\operatorname{Tr}(\hat m\, M)}_{\text{chiral mass}}
    ...
    + \underbrace{y_c\, H\, M^c{}_c
      + y_b\, H\, M^b{}_b
      + y_t\, H\, Q^t \bar Q_t\phantom{X}}_{\text{Yukawa}}\,,
\end{equation*}
%
where $M^{(3)}$ is the $3\times 3$ light-flavor block ($u,d,s$)
and $\Lambda_{\text{eff}}^6 = \Lambda^6/(m_u m_d m_s)$.
...
ISS magnetic dual, into a fixed magnetic Yukawa coupling $h = \sqrt{3}$.
This fixes the mass ratio $m_c/m_s = (2+\sqrt{3})^2/(2-\sqrt{3})^2$
...
splitting scale. The identification $\langle F\rangle \sim \fpi$
...
$m_b$, and $h=\sqrt{3}$ is determined, only two to three free
parameters remain:
\begin{itemize}
  ...
  \item $m_s$ (or equivalently $\Lambda_{\text{QCD}}$): the overall
    scale of the light sector.
\end{itemize}
The top mass $m_t$ is external---it does not participate in the
confined meson matrix. The Yukawa couplings $y_c = \sqrt{2}\,m_c/v_{\text{EW}}$,
$y_b = \sqrt{2}\,m_b/v_{\text{EW}}$ are fixed by the predicted masses.

The $(c,b,t)$ Koide triple provides an \emph{overdetermination test}:
with $m_c$ and $m_b$ already predicted and $m_t$ external,
$Q(c,b,t) = 0.670$ should be close to $2/3$---and it is
(0.42\% deviation). If both overlapping triples are required to
satisfy $Q = 2/3$ exactly, the overlap system determines
$m_b^{\text{pred}} = 4{,}159\MeV$ (0.5\% from PDG) and
$m_c^{\text{pred}} = 1{,}369\MeV$ (7.8\% from PDG).
...
Chiral color? \cite{FramptonGlashow1987} & $SU(3)_C$ & Axigluon mass \\
\bottomrule
\end{tabular}
\end{center}
...
either an algebraic image of the Miyazawa supercharge or a 
fundamental $SO(32)$ string state. The exotic $+4/3$-charged 
diquarks predicted by the $\mathbf{15}$ are a testable consequence.
...
$33\,\text{ppm}$ ($3.1\sigma$ after look-elsewhere over fractions
with denominator $\leq 20$). Since $2/9$
...
as a subgroup. The meson field $M$ carries $\mathbb{Z}_9$ charge 4
(from two quark zero modes at charge 2 each), and the baryon carries
charge 6. This embedding is specific to $N_c = 3$: the condition
$N_c^2 | 6N_c$ requires $N_c | 6$, which holds only for
$N_c = 1, 2, 3, 6$.
...
only on the modulus $|\Phi_0|$, not on the Koide phase. An ISS-type
extension with explicit $\mathbb{Z}_9$-breaking terms from the
instanton-generated $\mathbb{Z}_{18}$ is needed to
lift this flat direction.
...
the seed Koide fixes $m_c/m_s$ (from $h = \sqrt{3}$),
...
scale $m_s$. The remaining open question is the origin of the
$v_0$-doubling: it is non-holomorphic and cannot arise from the
superpotential. As argued in \S\ref{subsec:fterm}, fractional
instantons (monopole-instantons on $\mathbb{R}^3 \times S^1$)
produce exactly the required $\sum\sqrt{m_k}$ structure, with
the doubling emerging from the counting of active monopole amplitudes.
This mechanism operates in the K\"ahler sector and does not
conflict with the holomorphic superpotential. Its extension to
$\mathbb{R}^4$ is conjectural (via \"Unsal's continuity).
...
on-shell weak mixing angle to $0.13\sigma$, but three issues
...
$h = \sqrt{3}$ in magnetic dual & Derived & ISS dictionary \\
...
$v_0$-doubling: $v_0^{(f)}/v_0^{(s)} = 2.0005$ & Observed & Post-dictive \\
...
$v_0$-doubling is exact & Assumed & No mechanism yet \\
...
seed triple when $gv/m = \sqrt{3}$. The energy-balance condition
reduces to a single algebraic relation among the cubic coupling,
the pseudo-modulus VEV, and the mass parameter.
...
$\sin^2\theta_W = 0.22306$ to $0.13\sigma$ and giving
...
(\S\ref{subsec:superpotential}) comprises a canonical K\"ahler
potential, a superpotential combining the Seiberg constraint
with Yukawa couplings, $D$-terms with a Fayet--Iliopoulos
parameter, and soft breaking from a spurion
$\langle F\rangle = \fpi^2$.  The seed Koide fixes the magnetic
Yukawa coupling $h = \sqrt{3}$ (predicting $m_c/m_s$), and the
$v_0$-doubling fixes $m_b$---but as a non-holomorphic constraint
that must originate in the K\"ahler sector.  After these
...
$\mathbf{15}$ cannot be a $qq$ composite. It must be an
algebraically generated object (Miyazawa supercharge) or a
fundamental string-theoretic state ($SO(32)$ adjoint).
...
parametrization, at fixed vacuum energy $v_0$; it is not a
...
$h = \sqrt{3}$ in the magnetic dual) fixes the seed Koide ratio,
predicting $m_c$ from $m_s$;
(iii)~the vacuum-charge doubling predicts $m_b$---but this constraint
is non-holomorphic, arising from the K\"ahler sector rather than the
superpotential;
