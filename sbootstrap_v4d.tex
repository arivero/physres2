...
single constraint on the couplings. A Diophantine system on the
scalar content uniquely selects $N=3$ generations.
\end{abstract}
...
larger ($7.1\%$) discrepancy. The predicted triple satisfies the
...
not $g^2/(16\pi^2)$. This inversion is a signature of the 
dynamical (SQCD) realization versus the perturbative 
(superpotential) one.
...
triple except the leptons. No other quark triple comes close under
...
The total Lagrangian takes the schematic form
%
\begin{equation*}
  \mathcal{L}
  = \mathcal{L}_{\text{SQCD}}(V, Q, \bar Q)
  + \int\!d^2\theta\,\bigl[
      W_{\text{eff}}(M,B,\bar B)
      + W_{\text{Yukawa}}(H,Q,\bar Q)
    \bigr]
  + \mathcal{L}_{\text{FI}}(\xi, D_{\text{em}})\,,
\end{equation*}
%
where $\mathcal{L}_{\text{FI}}$ is a Fayet--Iliopoulos $D$-term for the
unbroken $U(1)_{\text{em}}$, providing an additional source of splitting
between charged and neutral states.

The parameters of this construction separate cleanly into those fixed by
structure and those that remain free.
%
\emph{Determined by the Koide ansatz}: within each mass triple, the ratios
$\zch_a/\zch_b = 2-\sqrt{3}$ fix two of three masses given the third.
%
\emph{Determined by SUSY breaking}: the supertrace relation
$m^2_\pi - m^2_\mu = \fpi^2$ fixes the splitting scale.
%
\emph{Free parameters}: the Koide phase $\delta$ for each triple
(lepton, light quark, heavy quark), the vacuum energy $v_0^2$ anchoring each
triple, the SUSY-breaking scale $f\approx\fpi$, and one $D$-term parameter
$\xi$---eight parameters in total. These determine ten independent masses:
three charged leptons, four quarks ($s,c,b,t$), and the meson triple
$(-\pi, D_s, B)$. The charm and bottom masses appear in both the $(-s,c,b)$
and $(c,b,t)$ triples, providing two nontrivial consistency conditions---the
overlapping triples must agree on $m_c$ and $m_b$. If both triples are
required to satisfy $Q = 2/3$ exactly, with only $m_s$ and $m_t$ as inputs,
the overlap system determines $m_c$ and $m_b$ uniquely:
$m_b^{\text{pred}} = 4{,}159\MeV$ (0.5\% from PDG) and
$m_c^{\text{pred}} = 1{,}369\MeV$ (7.8\% from PDG). The bottom mass
prediction is remarkably accurate; the charm mass deviation likely
reflects the fact that the quark triples satisfy $Q = 2/3$ only
approximately ($Q_{(-s,c,b)} = 0.675$, $Q_{(c,b,t)} = 0.670$).
The light quark masses $m_u$, $m_d$ are not constrained by any Koide
triple; they enter only through the GOR relation for $m_\pi$.
...
dynamics beyond the seed mechanism.
...
requiring $Q = 2/3$ on both overlapping triples predicts $m_b$ to
0.5\% from only $m_s$ and $m_t$ as inputs. The remaining question
is whether the Seiberg constraint (\S\ref{subsec:fterm}) and the
Yukawa superpotential can be unified into a single framework that
enforces both Koide conditions simultaneously. That all quark Koide
triples mix up-type and down-type mass eigenstates, and that the
free parameters left over are precisely $(m_u, m_d)$---connected
to the Cabibbo angle through the Oakes relation---may provide
a handle on this unification.
...
The entire charged fermion and pseudoscalar meson
...
$f_\pi$ that separates bosons from fermions. The
O'Raifeartaigh realization shows that the energy-balance condition
is a boundary condition on the superpotential ($gv = \sqrt{3}\,m$),
not an RG attractor. All quark Koide triples obligatorily mix
up-type and down-type mass eigenstates, and the residual free
parameters ($m_u$, $m_d$) connect to the CKM matrix through the
Oakes relation.
