...
Lepton masses in this framework must therefore arise from the
\emph{nonperturbative} K\"ahler sector (bion corrections), not
from the perturbative superpotential.  This is consistent with
the Koide condition being a boundary condition rather than
...
The scalar mass-squared matrix at the diagonal Seiberg vacuum
has six tachyonic eigenvalues.  The Yukawa-induced $F$-terms
($F_{M^d{}_d} = y_c v/\sqrt{2}$, $F_{M^s{}_s} = y_b v/\sqrt{2}$)
generate holomorphic third-derivative contributions $W_{IJK}F_I$
that overcome the positive soft mass $\tilde m^2 = \fpi^2$ in
the $us$ and $ds$ off-diagonal meson sectors, while the $ud$
sector remains stable.  The tachyonic directions correspond to
flavour-mixing meson condensates $\langle M^u{}_s\rangle,
\langle M^d{}_s\rangle \neq 0$, and the pattern mirrors the CKM
hierarchy: the larger instabilities involve strange--down and
strange--up mixing.  Finding the true vacuum---with off-diagonal
meson VEVs---may provide a dynamical origin for
Cabibbo-like flavour mixing within the Seiberg framework.
A global minimization of $V$ over all 14 real variables
(nine meson entries plus $X$, $B$, $\bar B$, $H_u^0$, $H_d^0$)
finds a minimum at $V \sim 2 \times 10^{13}\MeV^2$, a factor
$\sim 60$ below the diagonal point, but with order-one
mixing angles ($\theta_{12} \sim 34^\circ$, $\theta_{23} \sim 76^\circ$)---far
from the small CKM hierarchy.
The soft term $\tilde m^2\,\text{Tr}(M^\dagger M)$
dominates the vacuum energy, collapsing all $|M^i{}_j|$ to
$O(10^4\MeV)$ and destroying the seesaw hierarchy.
...
or by additional stabilization of the baryonic sector.
With baryons projected out, the off-diagonal condensates
should be computed perturbatively around the seesaw vacuum.
