...
on the Higgs field $H$, which is needed for the heavy sector where
...
\midrule
$m_{\text{lepton}}^2 = |\hat m_i + \hat m_j|^2$
  & $W$: fermion mass from $\partial^2 W/\partial M_i \partial M_j$ & $F$-term \\
$m_{\text{meson}}^2 = m_{\text{lepton}}^2 + \fpi^2$
  & $\mathcal{L}_{\text{soft}}$: $\tilde m^2 \sim \fpi^2$ & Spurion \\
$m_c/m_s = (2+\sqrt{3})^2/(2-\sqrt{3})^2$
  & $W$: O'Raifeartaigh at $h = \sqrt{3}$ & Seed Koide \\
$\sqrt{m_b} = 3\sqrt{m_s} + \sqrt{m_c}$
  & $K_{\text{bion}}$: minimum of $|S_{\text{bloom}} - 2S_{\text{seed}}|^2$
  & $v_0$-doubling \\
$m_t = y_t v_{\text{EW}}/\sqrt{2}$
  & $W_{\text{Yukawa}}$: $y_t H Q^t\bar Q_t$ & External \\
