...
$\sqrt{m_b} = 3\sqrt{m_s} + \sqrt{m_c}$, predicting
...
The stalled mass provides a striking algebraic consistency check.
...
This is not a weakness of the sBootstrap but a 
\emph{prediction}: the diquark sector requires new physics 
beyond the $qq$ picture.
...
five orders of magnitude below the physical $m_b = 4{,}180\MeV$.
...
\paragraph{The Koide condition is immune to $D$-terms.}
In $\mathcal{N}=1$ SUSY, $D$-terms shift scalar masses but leave
fermion masses untouched. Since the Koide triples are conditions
on \emph{fermion} masses, they are naturally protected against
$D$-term perturbations. The FI parameter $\xi$ affects only the
meson--lepton splittings, not the Koide ratios themselves.
...
$5\,\text{ppm}$ ($1.1\sigma$ given $\delta m_\tau$). Since $2/9$
...
extension with explicit $\mathbb{Z}_9$-breaking terms is needed to
...
superpotential. Whether the K\"ahler potential or nonperturbative
dynamics (instantons, gaugino condensation) enforces the doubling
is the key missing piece for a complete Lagrangian.
