...
K\"ahler sector; fractional instantons (monopole-instantons on
$\mathbb{R}^3\times S^1$) provide the required
$\sum\!\sqrt{m_k}$ structure. After these constraints, only two to three
...
of the form $|\!\sum s_k\sqrt{m_k}|^2$, which are exactly the $|v_0|^2$
terms needed for $v_0$-doubling. Organizing the bion amplitudes into
...
For $\mu = m_s = 93\MeV$, $h = \sqrt{3}$, and
$\Lambda_{\text{mag}} \sim 300\MeV$: $S_{\text{bounce}} \approx 950$,
giving a lifetime exceeding the age of the universe by $\sim 10^{247}$.
The critical mass at which the lifetime equals the Hubble time is
$\mu_{\text{crit}} \approx 117\MeV$ (with $h = \sqrt{3}$)---intriguingly
close to $m_s$, but safely below. The metastable vacuum is
...
masses into a near-Koide spectrum of meson condensates. However, the
...
\end{equation*}
%
with K\"ahler potential (canonical plus bion correction),
%
\begin{equation*}
  K = \underbrace{\operatorname{Tr}(M^\dagger M) + |X|^2 + |B|^2
    + |\bar B|^2 + |H|^2}_{K_{\text{can}}}
  + \underbrace{\frac{\zeta^2}{\Lambda^2}\,
    \Bigl|\sum_{k=1}^{N_c} s_k \sqrt{M^k{}_k}\,\Bigr|^2}_{K_{\text{bion}}}\,,
\end{equation*}
where $\zeta \propto e^{-S_0/(2N_c)}$ is the monopole fugacity and
$s_k = \pm 1$ are monopole-instanton signs (encoding the seed-to-bloom
phase). The bion term is non-holomorphic (containing $\sqrt{M}$) and
...
($\fpi^2$); (iii)~the Yukawa couplings for the heavy sector. It does
\emph{not} produce the bloom from seed to full triple: the $v_0$-doubling
is non-holomorphic and lives in the K\"ahler sector. The candidate
mechanism---monopole-instanton contributions on $\mathbb{R}^3 \times S^1$
(\S\ref{subsec:fterm})---generates the required $\sum\sqrt{m_k}$
structure, but its incorporation into a closed-form K\"ahler potential
remains open. The Lagrangian is therefore complete for the seed and
splitting, but sketches the bloom only through the monopole mechanism.
...
$v_0$-doubling is exact & Observed & Bion K\"ahler candidate \\
