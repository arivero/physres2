...
The off-diagonal meson scalars $M^d{}_s$, $M^s{}_d$ mediate
tree-level $\Delta S = 2$ transitions.  With a scalar mass
$m_S = \fpi = 92\MeV$, the Wilson coefficient is
$C_1 = g^2/(2\,m_S^2) \sim 6 \times 10^{-5}\MeV^{-2}$
for $g \sim 1$, which exceeds the UTfit bound
$|C_1| < 9 \times 10^{-13}\GeV^{-2}$~\cite{UTfit} by a factor
$\sim 70$.  The $K$--$\bar K$ constraint requires
$m_S \gtrsim 750\MeV$ (with $g = 1$), roughly $8\times$ larger
than~$\fpi$.  Several resolutions exist:
non-universal soft masses (with $m^2_{\text{off-diag}} \gg \fpi^2$),
GIM-like alignment of the meson--quark couplings, or
decoupling the off-diagonal scalars above the confinement scale.
A complete treatment requires computing the effective coupling
$g$ from the microscopic theory and including the off-diagonal
VEVs from the tachyonic condensate (\S\ref{sec:open}).
...
FCNC: $m_S = \fpi$ is $8\times$ too light & Derived & $K$-$\bar K$ mixing~\cite{UTfit} \\
\bottomrule
\end{tabular}
\end{center}
